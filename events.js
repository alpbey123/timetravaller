// TIME TRAVELLER — Event Database
// 109 events: T1-001–T1-030 | T2-001–T2-037 | T3-001–T3-042
//
// yaw: initial camera direction (0=forward, 90=right, 180=back, 270=left)

var EVENTS = [
  {
    id:         'T1-001',
    name:       "D-Day \u2013 Normandy Landings",
    year:       1944,
    lat:        49.3667,
    lng:        -0.8667,
    loc:        "Omaha Beach, Normandy",
    img:        'T1-001_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Omaha Beach, Normandy, France",
    desc:       "On June 6, 1944, over 156,000 Allied troops stormed five beaches along a 50-mile stretch of Normandy's heavily fortified coastline in the largest seaborne invasion in history. The operation, codenamed Overlord, succeeded at enormous cost \\\\u2014 nearly 10,000 Allied casualties on the first day alone \\\\u2014 and opened the long-awaited western front against Nazi Germany. Within three months, Paris was liberated; within a year, the war in Europe was over.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/a/a5/Into_the_Jaws_of_Death_23-0455M_edit.jpg",
    wikiTitle:  "Normandy landings"
  },
  {
    id:         'T1-002',
    name:       "Fall of Constantinople",
    year:       1453,
    lat:        41.0082,
    lng:        28.9784,
    loc:        "Constantinople (Istanbul)",
    img:        'T1-002_img1.png',
    yaw:        0,
    c1:         "Europe/Asia",
    c2:         "15th Century AD",
    c3:         "Constantinople (Istanbul), Turkey",
    desc:       "On May 29, 1453, Ottoman Sultan Mehmed II breached the walls of Constantinople after a 53-day siege, ending 1,123 years of Byzantine Roman rule and reshaping the political and religious map of the world. The city, defended by fewer than 10,000 men against an Ottoman force of 80,000, fell after the great chain across the Golden Horn was circumvented by rolling ships overland. The conquest marked the definitive end of the Middle Ages and opened a new era of Ottoman dominance across three continents.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/Siege_of_Constantinople_fresco%2C_Moldovi%C8%9Ba_monastery%2C_Vatra_Moldovi%C8%9Bei%2C_2017.jpg/3840px-Siege_of_Constantinople_fresco%2C_Moldovi%C8%9Ba_monastery%2C_Vatra_Moldovi%C8%9Bei%2C_2017.jpg",
    wikiTitle:  "Fall of Constantinople"
  },
  {
    id:         'T1-003',
    name:       "Eruption of Vesuvius \u2013 Destruction of Pompeii",
    year:       79,
    lat:        40.7491,
    lng:        14.4897,
    loc:        "Pompeii, Campania",
    img:        'T1-003_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "1st Century AD",
    c3:         "Pompeii, Campania, Italy",
    desc:       "In August 79 AD, Mount Vesuvius erupted with catastrophic force, burying the prosperous Roman city of Pompeii under four to six metres of volcanic ash and pumice in less than 24 hours. Pliny the Younger, who witnessed the eruption from across the Bay of Naples, described a terrifying column of ash rising into the sky \\\\u2014 the same cloud that killed his uncle Pliny the Elder as he attempted to rescue survivors. The city remained buried and largely intact for 1,700 years, becoming when rediscovered the most detailed snapshot of Roman daily life in existence.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/d/d1/Destruction_of_Pompeii_and_Herculaneum.jpg",
    wikiTitle:  "Eruption of Mount Vesuvius in 79 AD"
  },
  {
    id:         'T1-004',
    name:       "Storming of the Bastille \u2013 French Revolution",
    year:       1789,
    lat:        48.8533,
    lng:        2.3692,
    loc:        "Place de la Bastille, Paris",
    img:        'T1-004_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "18th Century AD",
    c3:         "Place de la Bastille, Paris, France",
    desc:       "On July 14, 1789, a Parisian crowd stormed the Bastille fortress \\\\u2014 a royal prison that symbolised royal tyranny \\\\u2014 seizing its gunpowder and releasing its captives. The assault came after days of mounting fury over food shortages, royal troops massing around Paris, and the dismissal of the popular finance minister Necker. The fall of the Bastille transformed a fiscal crisis into a revolution that would overthrow the monarchy, execute the king and queen, and rewrite the political foundations of the modern world.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/5/57/Anonymous_-_Prise_de_la_Bastille.jpg/3840px-Anonymous_-_Prise_de_la_Bastille.jpg",
    wikiTitle:  "Storming of the Bastille"
  },
  {
    id:         'T1-005',
    name:       "Great Fire of London",
    year:       1666,
    lat:        51.5074,
    lng:        -0.0983,
    loc:        "City of London",
    img:        'T1-005_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "17th Century AD",
    c3:         "City of London, United Kingdom",
    desc:       "The Great Fire of London began in a bakery on Pudding Lane on September 2, 1666, and burned for four days, destroying 13,200 houses, 87 churches, and most of the medieval City of London. The fire spread through the densely packed timber buildings, fanned by an unusually strong east wind during an extended drought. Though the disaster claimed only a handful of confirmed deaths, it displaced roughly 70,000 of the city's 80,000 inhabitants and provided the impetus for Christopher Wren's complete reimagining of London in stone and brick.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b6/Great_Fire_London.jpg/3840px-Great_Fire_London.jpg",
    wikiTitle:  "Great Fire of London"
  },
  {
    id:         'T1-006',
    name:       "Assassination of Julius Caesar",
    year:       -44,
    lat:        41.8955,
    lng:        12.4823,
    loc:        "Theatre of Pompey, Rome",
    img:        'T1-006_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "1st Century BC",
    c3:         "Theatre of Pompey, Rome, Italy",
    desc:       "On the Ides of March \\\\u2014 March 15, 44 BC \\\\u2014 a group of Roman senators led by Brutus and Cassius stabbed Julius Caesar 23 times in the Theatre of Pompey, believing they were saving the Republic from a tyrant who had declared himself dictator perpetuo. Caesar had been warned by a soothsayer and by his wife's premonitory dreams, but dismissed both and attended the Senate meeting as planned. The assassination achieved the opposite of its intended effect: the Republic never recovered, and the Roman Empire was born from the bloodshed.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/1/14/Vincenzo_Camuccini_-_La_morte_di_Cesare_%28cropped_3-2%29.jpg",
    wikiTitle:  "Assassination of Julius Caesar"
  },
  {
    id:         'T1-007',
    name:       "Black Death Reaches Western Europe",
    year:       1347,
    yearMin:    1347,
    yearMax:    1353,
    lat:        43.2965,
    lng:        5.3698,
    loc:        "Port of Marseille, France",
    img:        'T1-007_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "14th Century AD",
    c3:         "Port of Marseille, France, France",
    desc:       "In October 1347, twelve Genoese trading ships docked at the port of Messina, Sicily, carrying sailors already dying of a mysterious illness \\\\u2014 fever, swollen lymph nodes oozing blood and pus, and blackened skin. Within months the disease had spread across Mediterranean Europe, and over the following five years the Black Death killed between 30 and 50 percent of Europe's entire population \\\\u2014 the most lethal pandemic in human history. The plague reshaped European society permanently, triggering labour shortages that eroded serfdom and fuelling religious crisis.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/4/4e/1346-1353_spread_of_the_Black_Death_in_Europe_map.svg/1280px-1346-1353_spread_of_the_Black_Death_in_Europe_map.svg.png",
    wikiTitle:  "Black Death"
  },
  {
    id:         'T1-008',
    name:       "Battle of Waterloo",
    year:       1815,
    lat:        50.6796,
    lng:        4.4122,
    loc:        "Waterloo, Belgium",
    img:        'T1-008_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "19th Century AD",
    c3:         "Waterloo, Belgium, Belgium",
    desc:       "On June 18, 1815, near the Belgian village of Waterloo, the Duke of Wellington's Allied army held a defensive position against Napoleon Bonaparte's final offensive until Prussian reinforcements arrived in the late afternoon and turned the battle into a rout. Napoleon had staked everything on destroying the Allied forces before they could unite; the arrival of 50,000 Prussian troops under Bl\\\\u00fccher proved decisive. Napoleon abdicated four days later and was exiled to St Helena, ending twenty-six years of revolutionary and Napoleonic warfare.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/7/72/Battle_of_Waterloo_1815.PNG",
    wikiTitle:  "Battle of Waterloo"
  },
  {
    id:         'T1-009',
    name:       "Apollo 11 Moon Landing",
    year:       1969,
    lat:        28.5721,
    lng:        -80.648,
    loc:        "Kennedy Space Center, Florida, USA (mission launch site \u2014 event occurred on the Moon)",
    img:        'T1-009_img1.png',
    yaw:        0,
    c1:         "",
    c2:         "20th Century AD",
    c3:         "Kennedy Space Center, Florida, USA (mission launch site \u2014 event occurred on the Moon), USA",
    desc:       "On July 20, 1969, astronauts Neil Armstrong and Buzz Aldrin became the first humans to walk on the Moon, fulfilling a challenge set by President Kennedy just eight years earlier. Armstrong's first words as he stepped onto the lunar surface \\\\u2014 'That's one small step for a man, one giant leap for mankind' \\\\u2014 were broadcast live to an estimated 600 million people worldwide. The mission returned 21.5 kilograms of lunar rock samples and demonstrated that a democratic society could accomplish what had seemed a physical impossibility.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/4/41/A_Man_on_the_Moon%2C_AS11-40-5903_%28cropped%29.jpg/3840px-A_Man_on_the_Moon%2C_AS11-40-5903_%28cropped%29.jpg",
    wikiTitle:  "Apollo 11",
    moonEvent:  true
  },
  {
    id:         'T1-010',
    name:       "Atomic Bombing of Hiroshima",
    year:       1945,
    lat:        34.3955,
    lng:        132.4536,
    loc:        "Hiroshima, Japan",
    img:        'T1-010_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "20th Century AD",
    c3:         "Hiroshima, Japan, Japan",
    desc:       "On August 6, 1945, the United States dropped an atomic bomb called Little Boy on the Japanese city of Hiroshima, instantly killing between 70,000 and 80,000 people and destroying five square miles of the city centre; a second bomb fell on Nagasaki three days later. The two attacks remain the only uses of nuclear weapons in armed conflict, and together killed an estimated 129,000 to 226,000 people, the majority of them civilians. Japan announced its unconditional surrender on August 15, ending the Second World War and opening a nuclear age that permanently altered global conflict.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/5/54/Atomic_bombing_of_Japan.jpg/3840px-Atomic_bombing_of_Japan.jpg",
    wikiTitle:  "Atomic bombings of Hiroshima and Nagasaki"
  },
  {
    id:         'T1-011',
    name:       "Russian Revolution \u2013 Storming of the Winter Palace",
    year:       1917,
    lat:        59.9398,
    lng:        30.3146,
    loc:        "Winter Palace, Petrograd (St Petersburg)",
    img:        'T1-011_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Winter Palace, Petrograd (St Petersburg), Russia",
    desc:       "On the night of October 25, 1917, Bolshevik forces led by Lenin and Trotsky seized control of the Winter Palace in Petrograd, completing a near-bloodless coup that toppled Russia's Provisional Government. The storming was less dramatic than Soviet mythology later suggested \\\\u2014 a door was found unlocked and the ministers simply arrested \\\\u2014 but its consequences were monumental. The Bolshevik seizure of power established the world's first communist state and set in motion a political experiment that would define the twentieth century.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/b/b4/After_the_capture_of_the_Winter_Palace_26_October_1917.jpg",
    wikiTitle:  "October Revolution"
  },
  {
    id:         'T1-012',
    name:       "WWI \u2013 Battle of the Somme (Trench Warfare)",
    year:       1916,
    yearMin:    1916,
    yearMax:    1916,
    lat:        50.001,
    lng:        2.686,
    loc:        "Somme, Northern France",
    img:        'T1-012_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Somme, Northern France, France",
    desc:       "The Battle of the Somme, fought from July 1 to November 18, 1916, was one of the largest and costliest battles in human history, resulting in more than one million casualties along a 45-kilometre front in northern France. On the first day alone, the British Army suffered 57,470 casualties \\\\u2014 including 19,240 dead \\\\u2014 the single greatest loss in British military history. The battle achieved limited territorial gains and became the defining symbol of the industrial slaughter that characterised the First World War.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/a/ab/Map_of_the_Battle_of_the_Somme%2C_1916.svg/1280px-Map_of_the_Battle_of_the_Somme%2C_1916.svg.png",
    wikiTitle:  "Battle of the Somme"
  },
  {
    id:         'T1-013',
    name:       "WWII \u2013 Battle of Stalingrad",
    year:       1942,
    yearMin:    1942,
    yearMax:    1943,
    lat:        48.708,
    lng:        44.5133,
    loc:        "Stalingrad (Volgograd), Russia",
    img:        'T1-013_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Stalingrad (Volgograd), Russia, Russia",
    desc:       "The Battle of Stalingrad, fought from August 1942 to February 1943, was the turning point of the Second World War on the Eastern Front, ending with the encirclement and surrender of Germany's Sixth Army \\\\u2014 the first time a German field marshal had surrendered in the war. The city-by-city fighting reduced Stalingrad to rubble, with Soviet and German soldiers battling for individual buildings and staircases in what commanders called Rattenkrieg \\\\u2014 rat war. The German defeat cost nearly 800,000 Axis casualties and shifted the strategic momentum permanently in favour of the Soviet Union.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/5/5e/%D0%A0%D0%B0%D1%81%D1%87%D0%B5%D1%82_%D1%81%D0%BE%D0%B2%D0%B5%D1%82%D1%81%D0%BA%D0%BE%D0%B9_76%2C2-%D0%BC%D0%BC_%D0%BF%D1%83%D1%88%D0%BA%D0%B8_%D0%97%D0%B8%D0%A1-3_%D0%B2%D0%B5%D0%B4%D0%B5%D1%82_%D0%BE%D0%B3%D0%BE%D0%BD%D1%8C_%D0%B2_%D0%A1%D1%82%D0%B0%D0%BB%D0%B8%D0%BD%D0%B3%D1%80%D0%B0%D0%B4%D0%B5._%D0%9D%D0%BE%D1%8F%D0%B1%D1%80%D1%8C_1942_%D0%B3.jpg",
    wikiTitle:  "Battle of Stalingrad"
  },
  {
    id:         'T1-014',
    name:       "Attack on Pearl Harbor",
    year:       1941,
    lat:        21.3647,
    lng:        -157.9505,
    loc:        "Pearl Harbor, Oahu, Hawaii",
    img:        'T1-014_img1.png',
    yaw:        0,
    c1:         "Oceania/Pacific",
    c2:         "20th Century AD",
    c3:         "Pearl Harbor, Oahu, Hawaii, USA",
    desc:       "On the morning of December 7, 1941, 353 Japanese aircraft launched in two waves attacked the United States naval base at Pearl Harbor, sinking or damaging 21 ships, destroying 188 aircraft, and killing 2,403 Americans. Japan aimed to neutralise the US Pacific Fleet long enough to complete the conquest of Southeast Asia's resources; the attack instead unified American public opinion overnight. President Roosevelt called it 'a date which will live in infamy,' and the United States declared war the following day, transforming a European conflict into a truly global war.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c7/Attack_on_Pearl_Harbor_Japanese_planes_view.jpg/3840px-Attack_on_Pearl_Harbor_Japanese_planes_view.jpg",
    wikiTitle:  "Attack on Pearl Harbor"
  },
  {
    id:         'T1-015',
    name:       "Fall of the Berlin Wall",
    year:       1989,
    lat:        52.5163,
    lng:        13.3777,
    loc:        "Brandenburg Gate, Berlin",
    img:        'T1-015_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Brandenburg Gate, Berlin, Germany",
    desc:       "On November 9, 1989, following a confused announcement by an East German spokesman, crowds gathered at Berlin Wall checkpoints and guards \\\\u2014 receiving no orders to resist \\\\u2014 opened the gates. Berliners from both sides embraced, wept, and began dismantling the wall with hammers and pickaxes, ending 28 years of physical division during which 140 people had been killed attempting to cross. German reunification followed less than a year later, the Soviet Union dissolved by 1991, and the Cold War was effectively over.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/1/1c/West_and_East_Germans_at_the_Brandenburg_Gate_in_1989.jpg",
    wikiTitle:  "Fall of the Berlin Wall"
  },
  {
    id:         'T1-016',
    name:       "Crucifixion of Jesus Christ",
    year:       30,
    yearMin:    30,
    yearMax:    33,
    lat:        31.7784,
    lng:        35.2296,
    loc:        "Golgotha (Church of the Holy Sepulchre), Jerusalem",
    img:        'T1-016_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "1st Century AD",
    c3:         "Golgotha (Church of the Holy Sepulchre), Jerusalem, Israel",
    desc:       "Around 30 AD, Jesus of Nazareth was crucified outside Jerusalem's walls at Golgotha on the orders of the Roman prefect Pontius Pilate, following his arrest in the Garden of Gethsemane. All four Gospels record an unnatural darkness for three hours during the crucifixion, and the event has been commemorated each year on Good Friday by approximately 2.4 billion Christians worldwide for nearly two millennia. No event in human history has generated more art, music, theology, and philosophical reflection than the execution of this itinerant preacher from Galilee.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/d/d7/Cristo_crucificado.jpg",
    wikiTitle:  "Crucifixion of Jesus"
  },
  {
    id:         'T1-017',
    name:       "Napoleon's Coronation as Emperor",
    year:       1804,
    lat:        48.853,
    lng:        2.3499,
    loc:        "Notre-Dame Cathedral, Paris",
    img:        'T1-017_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "19th Century AD",
    c3:         "Notre-Dame Cathedral, Paris, France",
    desc:       "On December 2, 1804, Napoleon Bonaparte crowned himself Emperor of the French in a ceremony at Notre-Dame Cathedral before Pope Pius VII \\\\u2014 a deliberate inversion of tradition in which the conqueror placed the crown on his own head rather than receiving it from the Church. The painter Jacques-Louis David immortalised the scene in a vast canvas showing Napoleon crowning his empress Jos\\\\u00e9phine. The coronation announced to Europe that the French Revolution had produced not just a republic but a new kind of monarch \\\\u2014 one whose authority derived from military genius and popular will, not divine right.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1e/Jacques-Louis_David_-_The_Coronation_of_Napoleon_%281805-1807%29.jpg/3840px-Jacques-Louis_David_-_The_Coronation_of_Napoleon_%281805-1807%29.jpg",
    wikiTitle:  "Coronation of Napoleon"
  },
  {
    id:         'T1-018',
    name:       "Battle of Gettysburg",
    year:       1863,
    yearMin:    1863,
    yearMax:    1863,
    lat:        39.8112,
    lng:        -77.2253,
    loc:        "Gettysburg, Pennsylvania",
    img:        'T1-018_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "19th Century AD",
    c3:         "Gettysburg, Pennsylvania, USA",
    desc:       "The Battle of Gettysburg, fought July 1\\\\u20133, 1863, was the largest and most costly battle of the American Civil War, resulting in approximately 51,000 casualties across three days of fighting in the Pennsylvania countryside. Confederate General Robert E. Lee's attempted invasion of the North was turned back by Union forces under General George Meade, ending any realistic prospect of European recognition for the Confederacy. President Lincoln's Gettysburg Address, delivered four months later, reframed the war as a struggle not merely for union but for the principle that all men are created equal.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c7/Thure_de_Thulstrup_-_L._Prang_and_Co._-_Battle_of_Gettysburg_-_Restoration_by_Adam_Cuerden.jpg/3840px-Thure_de_Thulstrup_-_L._Prang_and_Co._-_Battle_of_Gettysburg_-_Restoration_by_Adam_Cuerden.jpg",
    wikiTitle:  "Battle of Gettysburg"
  },
  {
    id:         'T1-019',
    name:       "Cuban Missile Crisis \u2013 Naval Blockade",
    year:       1962,
    yearMin:    1962,
    yearMax:    1962,
    lat:        23.1136,
    lng:        -82.3666,
    loc:        "Havana / Caribbean Sea, Cuba",
    img:        'T1-019_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "20th Century AD",
    c3:         "Havana / Caribbean Sea, Cuba, Cuba",
    desc:       "On October 16, 1962, US reconnaissance photographs revealed Soviet nuclear missile installations under construction in Cuba, capable of striking Washington DC in thirteen minutes \\\\u2014 triggering thirteen days of secret deliberation that brought the world closer to nuclear war than at any point before or since. Kennedy opted for a naval blockade rather than the air strike recommended by his generals; Soviet ships carrying further missile components turned back from the quarantine line on October 24. The crisis was resolved when the Soviets agreed to remove the missiles in exchange for a US pledge not to invade Cuba.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/8/8a/PGM-19A_Jupiter_missile-02.jpg",
    wikiTitle:  "Cuban Missile Crisis"
  },
  {
    id:         'T1-020',
    name:       "Liberation of Paris \u2013 WWII",
    year:       1944,
    yearMin:    1944,
    yearMax:    1944,
    lat:        48.8738,
    lng:        2.295,
    loc:        "Champs-\u00c9lys\u00e9es, Paris",
    img:        'T1-020_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Champs-\u00c9lys\u00e9es, Paris, France",
    desc:       "Between August 19 and 25, 1944, French and Allied forces liberated Paris from four years of German occupation, with General Charles de Gaulle leading a triumphant procession down the Champs-\\\\u00c9lys\\\\u00e9es on August 26 before a crowd of hundreds of thousands despite sniper fire still echoing across rooftops. Hitler had ordered the military governor of Paris to burn the city's bridges and monuments; General Dietrich von Choltitz defied the order and surrendered with the city largely intact. The liberation restored French national dignity and signalled that the German defeat in Western Europe was now a matter of months.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/7/78/Crowds_of_French_patriots_line_the_Champs_Elysees-edit2.jpg/3840px-Crowds_of_French_patriots_line_the_Champs_Elysees-edit2.jpg",
    wikiTitle:  "Liberation of Paris"
  },
  {
    id:         'T1-021',
    name:       "Dunkirk Evacuation \u2013 Operation Dynamo",
    year:       1940,
    yearMin:    1940,
    yearMax:    1940,
    lat:        51.0344,
    lng:        2.3767,
    loc:        "Dunkirk beach, Northern France",
    img:        'T1-021_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Dunkirk beach, Northern France, France",
    desc:       "Between May 26 and June 4, 1940, Operation Dynamo evacuated 338,226 Allied soldiers from the beaches of Dunkirk as German forces closed in \\\\u2014 a rescue mounted by 861 vessels including Royal Navy warships and hundreds of private 'little ships' whose owners sailed them across the Channel themselves. Prime Minister Churchill had privately feared rescuing no more than 30,000 men; the actual number exceeded all expectations and preserved the core of the British Army to fight on. Churchill warned that wars are not won by evacuations, but Dunkirk transformed a military catastrophe into a founding myth of British national resilience.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/8/88/Dunkirk_26-29_May_1940_NYP68075.jpg/3840px-Dunkirk_26-29_May_1940_NYP68075.jpg",
    wikiTitle:  "Dunkirk evacuation"
  },
  {
    id:         'T1-022',
    name:       "WWI Christmas Truce",
    year:       1914,
    yearMin:    1914,
    yearMax:    1914,
    lat:        50.73,
    lng:        2.9,
    loc:        "Western Front, Ypres Salient, Belgium",
    img:        'T1-022_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Western Front, Ypres Salient, Belgium, Belgium",
    desc:       "On Christmas Eve 1914, in the frozen trenches of the Western Front near Ypres, German soldiers began placing candles and small Christmas trees along their parapets and singing carols \\u2014 prompting British troops opposite them to emerge cautiously from their trenches and meet in No Man's Land for an informal truce that neither government had sanctioned. The men exchanged cigarettes, shared food, and in some documented cases played impromptu football matches on the cratered ground between positions they had been trying to kill each other from for months. The Christmas Truce, involving an estimated 100,000 soldiers along a significant portion of the front, became the most famous spontaneous act of humanity in the history of industrial warfare \\u2014 and was never repeated at the same scale as the war hardened and the casualties mounted.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/en/4/42/Illustrated_London_News_-_Christmas_Truce_1914.jpg",
    wikiTitle:  "Christmas truce"
  },
  {
    id:         'T1-023',
    name:       "Opening of the Eiffel Tower",
    year:       1889,
    lat:        48.8584,
    lng:        2.2945,
    loc:        "Champ de Mars, Paris, France",
    img:        'T1-023_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "19th Century AD",
    c3:         "Champ de Mars, Paris, France, France",
    desc:       "On March 31, 1889, the Eiffel Tower was inaugurated as the entrance arch to the 1889 World's Fair in Paris, having been constructed in just over two years by Gustave Eiffel's engineering company using 18,038 pieces of wrought iron held together by 2.5 million rivets \u2014 a feat of industrial precision so radical that a committee of 300 artists and intellectuals had petitioned the city to have it demolished before it was finished. At 330 metres it was the tallest man-made structure on Earth for 41 years, until the Chrysler Building surpassed it in 1930. The tower, designed as a temporary structure to be torn down after 20 years, became so useful as a radio transmission antenna that it was preserved \u2014 and is now the most visited paid monument on Earth, welcoming approximately 7 million visitors per year.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/8/85/Eiffel_Tower_from_the_Champ_de_Mars%2C_Paris%2C_August_2021.jpg",
    wikiTitle:  "Eiffel Tower"
  },
  {
    id:         'T1-024',
    name:       "Darwin Publishes 'On the Origin of Species'",
    year:       1859,
    lat:        51.2946,
    lng:        0.0431,
    loc:        "Down House, Downe, Kent, England",
    img:        'T1-024_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "19th Century AD",
    c3:         "Down House, Downe, Kent, England, United Kingdom",
    desc:       "On November 24, 1859, John Murray published 1,250 copies of Charles Darwin's On the Origin of Species \u2014 all sold on the first day \u2014 presenting the theory of evolution by natural selection that Darwin had been developing for over twenty years, ever since his observations aboard HMS Beagle had forced him to question the fixity of species. The book argued that all life on Earth is descended from common ancestors, and that the diversity of species arises through the gradual accumulation of heritable variations that improve survival and reproduction over vast stretches of time. No scientific work since Newton's Principia has more fundamentally transformed humanity's understanding of its place in the universe, and no idea in the history of science has met with more ferocious cultural and theological resistance before achieving near-universal scientific acceptance.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/Origin_of_Species.jpg/256px-Origin_of_Species.jpg",
    wikiTitle:  "On the Origin of Species"
  },
  {
    id:         'T1-025',
    name:       "First Modern Olympic Games",
    year:       1896,
    yearMin:    1896,
    yearMax:    1896,
    lat:        37.9684,
    lng:        23.7286,
    loc:        "Panathenaic Stadium, Athens, Greece",
    img:        'T1-025_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "19th Century AD",
    c3:         "Panathenaic Stadium, Athens, Greece, Greece",
    desc:       "On April 6, 1896, the first modern Olympic Games opened at the restored Panathenaic Stadium in Athens \u2014 built on the site of an ancient Greek stadium and clad entirely in white marble \u2014 with 241 athletes from 14 nations competing in 43 events over ten days, nine of the fourteen nations winning at least one medal. The Games had been revived by the French baron Pierre de Coubertin, who believed competitive sport could promote international understanding and peace; the Greek public was so enthusiastic that the organisers seriously debated relocating the permanent games to Athens rather than rotating between host cities. The modern Olympics have grown from those 241 athletes to over 10,000 competing in more than 300 events, becoming the largest peacetime gathering of nations in human history.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/8/8d/1896_Olympic_opening_ceremony.jpg/640px-1896_Olympic_opening_ceremony.jpg",
    wikiTitle:  "1896 Summer Olympics"
  },
  {
    id:         'T1-026',
    name:       "Woodstock Music Festival",
    year:       1969,
    yearMin:    1969,
    yearMax:    1969,
    lat:        41.6965,
    lng:        -74.8783,
    loc:        "Bethel, New York (Max Yasgur's Farm)",
    img:        'T1-026_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "20th Century AD",
    c3:         "Bethel, New York (Max Yasgur's Farm), USA",
    desc:       "On August 15\u201318, 1969, approximately 400,000 people gathered on Max Yasgur's dairy farm near Bethel, New York, for the Woodstock Music and Art Fair \u2014 three days of music, mud, and improvised community that became the defining cultural event of the 1960s counterculture, featuring performances by Jimi Hendrix, Janis Joplin, The Who, Jefferson Airplane, Crosby Stills Nash & Young, and thirty other acts. The infrastructure collapsed within hours: the food ran out, the roads gridlocked for miles, and the organizers were forced to declare it a free concert, losing millions of dollars. Yet what emerged \u2014 peaceful coexistence among half a million people under impossible conditions, sharing food and shelter with strangers \u2014 seemed to many participants to prove that the generation's values of community and nonviolence were not merely utopian.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/0/09/WoodstockNationalHistoricLandmarkSign.jpg/640px-WoodstockNationalHistoricLandmarkSign.jpg",
    wikiTitle:  "Woodstock"
  },
  {
    id:         'T1-027',
    name:       "Inauguration of Nelson Mandela as President",
    year:       1994,
    lat:        -25.7046,
    lng:        28.2318,
    loc:        "Union Buildings, Pretoria, South Africa",
    img:        'T1-027_img1.png',
    yaw:        0,
    c1:         "Africa",
    c2:         "20th Century AD",
    c3:         "Union Buildings, Pretoria, South Africa, South Africa",
    desc:       "On May 10, 1994, Nelson Mandela was inaugurated as the first democratically elected President of South Africa at the Union Buildings in Pretoria \u2014 the same government complex from which the apartheid state had been administered for decades \u2014 in a ceremony attended by the leaders of more than 140 countries, the largest gathering of world leaders in history to that date. Mandela had spent 27 years in prison, emerged without apparent bitterness, negotiated the transition from apartheid with his former jailers, and now stood before the world as the embodiment of what political reconciliation could achieve. His inaugural address \u2014 'Never, never, and never again shall it be that this beautiful land will again experience the oppression of one by another' \u2014 was broadcast to hundreds of millions worldwide.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/0/02/Nelson_Mandela_1994.jpg/400px-Nelson_Mandela_1994.jpg",
    wikiTitle:  "Inauguration of Nelson Mandela"
  },
  {
    id:         'T1-028',
    name:       "Opening of the Suez Canal",
    year:       1869,
    lat:        30.5852,
    lng:        32.2654,
    loc:        "Ismailia, Egypt",
    img:        'T1-028_img1.png',
    yaw:        0,
    c1:         "Africa",
    c2:         "19th Century AD",
    c3:         "Ismailia, Egypt, Egypt",
    desc:       "On November 17, 1869, the Suez Canal was inaugurated with a grand procession of ships through its 193-kilometre length connecting the Mediterranean to the Red Sea \u2014 a waterway that had taken ten years and an estimated 1.5 million forced labourers to dig through the Isthmus of Suez, at a cost of over 100 human lives per kilometre. The canal reduced the sea voyage from Britain to India by approximately 7,000 kilometres, transforming global trade routes, eliminating the need to circumnavigate Africa, and making the Middle East strategically indispensable to European empires. Its nationalisation by Egyptian President Nasser in 1956 triggered the Suez Crisis \u2014 a defining moment in the collapse of British and French imperial power and the emergence of American hegemony.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/9/95/SuezCanal-EO.JPG/640px-SuezCanal-EO.JPG",
    wikiTitle:  "Suez Canal"
  },
  {
    id:         'T1-029',
    name:       "Alexander Graham Bell Makes the First Telephone Call",
    year:       1876,
    lat:        42.3601,
    lng:        -71.0589,
    loc:        "5 Exeter Place, Boston, Massachusetts",
    img:        'T1-029_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "19th Century AD",
    c3:         "5 Exeter Place, Boston, Massachusetts, USA",
    desc:       "On March 10, 1876, in a rented laboratory at 5 Exeter Place in Boston, Alexander Graham Bell spoke the first words ever transmitted by telephone to his assistant Thomas Watson in the next room: 'Mr Watson, come here \u2014 I want to see you.' Bell had filed his patent for 'the method of, and apparatus for, transmitting vocal or other sounds telegraphically' just two hours before a rival inventor, Elisha Gray, filed a competing claim \u2014 one of the most consequential near-misses in technological history. The telephone Bell demonstrated at the Philadelphia Centennial Exhibition later that year astonished audiences who had never imagined the human voice could travel through a wire, and within fifteen years over 150,000 telephones were in use across America.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9c/Alexander_Graham_Bell_1895_NPG_77_363.jpg/3840px-Alexander_Graham_Bell_1895_NPG_77_363.jpg",
    wikiTitle:  "Alexander Graham Bell"
  },
  {
    id:         'T1-030',
    name:       "Fall of the Soviet Union \u2013 Gorbachev's Resignation",
    year:       1991,
    lat:        55.7517,
    lng:        37.6173,
    loc:        "Kremlin, Moscow, Russia",
    img:        'T1-030_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Kremlin, Moscow, Russia, Russia",
    desc:       "On December 25, 1991, Mikhail Gorbachev delivered his resignation address as President of the Soviet Union and signed the decree transferring control of the Soviet nuclear arsenal to Boris Yeltsin, formally ending 69 years of communist rule and dissolving a state that had covered one-sixth of the Earth's land surface and possessed thousands of nuclear warheads. The Soviet flag was lowered over the Kremlin for the last time and replaced by the Russian tricolour; the fifteen Soviet republics became independent nations overnight. The dissolution, which had been accelerated by Gorbachev's own reforms of glasnost and perestroika, ended the Cold War that had structured global politics for four decades \u2014 though the consequences of that sudden collapse, including the wars, economic devastation, and democratic backsliding that followed, would define the next thirty years.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a9/Soviet_flag_waving.jpg/640px-Soviet_flag_waving.jpg",
    wikiTitle:  "Dissolution of the Soviet Union"
  },
  {
    id:         'T2-001',
    name:       "Battle of Thermopylae",
    year:       -480,
    lat:        38.7956,
    lng:        22.5339,
    loc:        "Thermopylae Pass, Greece",
    img:        'T2-001_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "5th Century BC",
    c3:         "Thermopylae Pass, Greece, Greece",
    desc:       "In 480 BC, a Greek force of approximately 7,000 soldiers \\\\u2014 including 300 Spartan warriors under King Leonidas \\\\u2014 held the narrow mountain pass at Thermopylae against an invading Persian army for two days, until a local shepherd revealed a mountain path that allowed the Persians to surround them. Leonidas dismissed most of the Greek contingent before the final stand, remaining with 300 Spartans and others to fight to the last man and buy time for the Greek fleet at Salamis. The battle became the most celebrated rearguard action in military history, its defenders immortalised by the epitaph: 'Go tell the Spartans, stranger passing by, that here, obedient to their laws, we lie.'",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/9/91/L%C3%A9onidas_aux_Thermopyles_-_Jacques-Louis_David_-_Mus%C3%A9e_du_Louvre_Peintures_INV_3690_%3B_L_3711.jpg/3840px-L%C3%A9onidas_aux_Thermopyles_-_Jacques-Louis_David_-_Mus%C3%A9e_du_Louvre_Peintures_INV_3690_%3B_L_3711.jpg",
    wikiTitle:  "Battle of Thermopylae"
  },
  {
    id:         'T2-002',
    name:       "Mongol Sack of Baghdad",
    year:       1258,
    lat:        33.3406,
    lng:        44.4009,
    loc:        "Baghdad, Iraq",
    img:        'T2-002_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "13th Century AD",
    c3:         "Baghdad, Iraq, Iraq",
    desc:       "In February 1258, the Mongol army of Hulagu Khan besieged Baghdad, capital of the Abbasid Caliphate and the intellectual heart of the Islamic world, then home to the largest library on earth \\\\u2014 the House of Wisdom. After a siege of less than two weeks, the city fell; the Caliph was executed and the city was sacked for seventeen days. Contemporary chronicles described the Tigris running black with the ink of books thrown into it and red with the blood of scholars \\\\u2014 a catastrophe from which the Islamic Golden Age of science and philosophy never fully recovered.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/8/85/Bagdad1258.jpg",
    wikiTitle:  "Siege of Baghdad"
  },
  {
    id:         'T2-003',
    name:       "Signing of Magna Carta",
    year:       1215,
    lat:        51.439,
    lng:        -0.568,
    loc:        "Runnymede, Surrey, England",
    img:        'T2-003_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "13th Century AD",
    c3:         "Runnymede, Surrey, England, United Kingdom",
    desc:       "On June 15, 1215, King John of England affixed his seal to Magna Carta \\\\u2014 the Great Charter \\\\u2014 at Runnymede by the River Thames, conceding for the first time that even a king was subject to the rule of law and could not imprison his subjects without lawful judgement. The charter was forced upon John by rebellious barons outraged by his arbitrary taxation, and he repudiated it within weeks; but it was reissued and reconfirmed over thirty times by subsequent monarchs. Its clauses guaranteeing due process became the bedrock of constitutional democracy in Britain, the United States, and every legal system derived from English common law.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/e/ee/Magna_Carta_%28British_Library_Cotton_MS_Augustus_II.106%29.jpg/3840px-Magna_Carta_%28British_Library_Cotton_MS_Augustus_II.106%29.jpg",
    wikiTitle:  "Magna Carta"
  },
  {
    id:         'T2-004',
    name:       "Battle of Hastings",
    year:       1066,
    lat:        50.9098,
    lng:        0.4876,
    loc:        "Senlac Hill, Battle, East Sussex",
    img:        'T2-004_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "11th Century AD",
    c3:         "Senlac Hill, Battle, East Sussex, United Kingdom",
    desc:       "On October 14, 1066, William, Duke of Normandy, defeated King Harold II of England near Hastings in a battle that lasted the entire day \\\\u2014 ending when Harold was killed, reportedly struck in the eye by an arrow, as his shield wall finally broke under repeated Norman cavalry charges. William's victory was total: England's entire ruling class was replaced within a generation, the English language absorbed thousands of French words, and the country's architecture, law, and culture were transformed beyond recognition. The Norman Conquest was the last successful invasion of England, and its consequences shaped the English-speaking world.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/b/bb/Bayeux_Tapestry_scene57_Harold_death.jpg",
    wikiTitle:  "Battle of Hastings"
  },
  {
    id:         'T2-005',
    name:       "Boston Tea Party",
    year:       1773,
    lat:        42.3536,
    lng:        -71.05,
    loc:        "Griffin's Wharf, Boston",
    img:        'T2-005_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "18th Century AD",
    c3:         "Griffin's Wharf, Boston, USA",
    desc:       "On the night of December 16, 1773, the Sons of Liberty \\\\u2014 American colonists disguised as Mohawk Indians \\\\u2014 boarded three British East India Company ships moored in Boston Harbour and dumped 342 chests of tea worth approximately \\\\u00a310,000 into the water, protesting taxation without colonial representation. Britain's furious response \\\\u2014 the Coercive Acts, which closed Boston's port and revoked Massachusetts' self-governance \\\\u2014 united the colonies in resistance and led directly to the First Continental Congress. What began as a dispute over tea duty ended eighteen months later with shots fired at Lexington and Concord.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/e/e6/Boston_Tea_Party_w.jpg/3840px-Boston_Tea_Party_w.jpg",
    wikiTitle:  "Boston Tea Party"
  },
  {
    id:         'T2-006',
    name:       "Columbus Reaches the Americas",
    year:       1492,
    lat:        24.05,
    lng:        -74.5333,
    loc:        "San Salvador Island, Bahamas",
    img:        'T2-006_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "15th Century AD",
    c3:         "San Salvador Island, Bahamas, Bahamas",
    desc:       "On October 12, 1492, after 36 days at sea with a crew on the edge of mutiny, Christopher Columbus made landfall on a Caribbean island he named San Salvador \\\\u2014 believing until his death that he had reached the outlying islands of Asia. Columbus's four voyages set in motion the permanent connection between the Eastern and Western hemispheres \\\\u2014 the Columbian Exchange \\\\u2014 that transformed the food, disease, economy, and population of every continent on Earth. The encounter initiated a century of Spanish conquest and colonisation that would devastate indigenous civilisations and reshape the Americas irrevocably.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/2/23/Viajes_de_colon_en2.svg/960px-Viajes_de_colon_en2.svg.png",
    wikiTitle:  "Voyages of Christopher Columbus"
  },
  {
    id:         'T2-007',
    name:       "Sack of Rome by the Visigoths",
    year:       410,
    lat:        41.8892,
    lng:        12.4921,
    loc:        "Rome, Italy",
    img:        'T2-007_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "5th Century AD",
    c3:         "Rome, Italy, Italy",
    desc:       "In August 410 AD, the Visigoth king Alaric led his forces through the Salarian Gate into Rome \\\\u2014 the first time the city had been sacked by a foreign enemy in 800 years \\\\u2014 and allowed three days of looting before ordering the churches and their refugees spared. The psychological shock was enormous: the philosopher Jerome wrote that 'the world is rushing headlong to its ruin'; Augustine of Hippo began writing The City of God in direct response. Though relatively restrained by ancient standards, the sack shattered the myth of Rome's invincibility and signalled the beginning of the Western Empire's final collapse.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/6/61/Visigoths_sack_Rome.jpg",
    wikiTitle:  "Sack of Rome (410)"
  },
  {
    id:         'T2-008',
    name:       "Gutenberg Prints the Bible",
    year:       1454,
    yearMin:    1454,
    yearMax:    1456,
    lat:        49.9929,
    lng:        8.2473,
    loc:        "Mainz, Germany",
    img:        'T2-008_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "15th Century AD",
    c3:         "Mainz, Germany, Germany",
    desc:       "Around 1455, Johannes Gutenberg of Mainz completed printing the first substantial book produced using movable metal type in Europe \\\\u2014 the 42-line Gutenberg Bible \\\\u2014 using a printing press he had secretly developed over nearly a decade. Approximately 180 copies were printed, of which 49 survive; contemporaries marvelled that the pages looked as precise as handwritten script but were produced at unimaginable speed. The printing press is widely regarded as the most transformative technology in human history \\\\u2014 enabling the Reformation, the Scientific Revolution, and the standardisation of national languages by making books affordable for the first time.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/5/5b/Mainz_Gutenbergdenkmal_2016_%28cropped%29.jpg",
    wikiTitle:  "Johannes Gutenberg"
  },
  {
    id:         'T2-009',
    name:       "Battle of Gaugamela \u2013 Alexander vs Darius III",
    year:       -331,
    lat:        36.36,
    lng:        43.25,
    loc:        "Gaugamela, near Mosul, Iraq",
    img:        'T2-009_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "4th Century BC",
    c3:         "Gaugamela, near Mosul, Iraq, Iraq",
    desc:       "On October 1, 331 BC, Alexander the Great met the Persian Emperor Darius III on a flat plain near Gaugamela \\\\u2014 a site Darius had chosen specifically to give his scythed chariots and vastly superior numbers room to manoeuvre. Alexander shattered the Persian line with a precisely timed cavalry charge into a gap created by Darius's own advancing forces, driving Darius from the field in flight and leaving his army leaderless. The victory gave Alexander control of the Persian Empire and the title King of Asia \\\\u2014 the greatest conquest achieved by a single commander in antiquity.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/0/0b/Pietro_da_Cortona_-_Battle_of_Alexander_versus_Darius_-_Google_Art_Project.jpg",
    wikiTitle:  "Battle of Gaugamela"
  },
  {
    id:         'T2-010',
    name:       "Haitian Revolution \u2013 Declaration of Independence",
    year:       1804,
    lat:        19.76,
    lng:        -72.2,
    loc:        "Gona\u00efves, Haiti",
    img:        'T2-010_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "19th Century AD",
    c3:         "Gona\u00efves, Haiti, Haiti",
    desc:       "The Haitian Revolution, which began in 1791 as a slave uprising in the French colony of Saint-Domingue and ended on January 1, 1804 with the declaration of the Republic of Haiti, was the only successful slave revolt in human history to produce an independent nation. Led by Jean-Jacques Dessalines, the Haitian forces defeated Napoleon's army in a campaign that Napoleon's own brother-in-law compared to fighting tigers. Haiti's independence terrified slaveholding societies across the Americas and became the most powerful symbol of Black resistance to slavery and colonialism.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/4/44/Incendie_de_la_Plaine_du_Cap._Massacre_des_Blancs_par_les_esclaves_noirs_r%C3%A9volt%C3%A9s._France_militaire._Martinet_et_Masson.jpg",
    wikiTitle:  "Haitian Revolution"
  },
  {
    id:         'T2-011',
    name:       "Charlemagne's Coronation as Holy Roman Emperor",
    year:       800,
    lat:        41.9022,
    lng:        12.4539,
    loc:        "Old St. Peter's Basilica, Rome",
    img:        'T2-011_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "8th Century AD",
    c3:         "Old St. Peter's Basilica, Rome, Italy",
    desc:       "On Christmas Day 800 AD, Pope Leo III placed a crown on the head of Charlemagne \\\\u2014 King of the Franks and ruler of most of Western Europe \\\\u2014 in Old St Peter's Basilica in Rome, crowning him Emperor of the Romans and creating what would become the Holy Roman Empire. Charlemagne later claimed the coronation surprised him; modern historians debate whether this was false modesty or genuine irritation at the political implications of receiving authority from the Pope. The coronation established the political template of Christian Europe for the next thousand years \\\\u2014 the idea that secular and religious power were inseparably entwined.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/f/fb/Charlemagne_denier_Mayence_812_814.jpg",
    wikiTitle:  "Charlemagne"
  },
  {
    id:         'T2-012',
    name:       "Great Schism \u2013 Excommunication of the Orthodox Patriarch",
    year:       1054,
    lat:        41.0086,
    lng:        28.9802,
    loc:        "Hagia Sophia, Constantinople",
    img:        'T2-012_img1.png',
    yaw:        0,
    c1:         "Europe/Asia",
    c2:         "11th Century AD",
    c3:         "Hagia Sophia, Constantinople, Turkey",
    desc:       "On July 16, 1054, Cardinal Humbert placed a bull of excommunication on the altar of Hagia Sophia in Constantinople, excommunicating the Patriarch of Constantinople \\\\u2014 who responded by excommunicating Humbert and the papal legates in return. The dispute, ostensibly over the use of unleavened bread in the Eucharist and the authority of the papacy, had been building for centuries as Rome and Constantinople had drifted apart theologically, politically, and culturally. The mutual excommunications formalised the Great Schism \\\\u2014 the division of Christianity into Roman Catholic and Eastern Orthodox communions that has persisted to the present day.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/9/90/Great_Schism_with_former_borders_%281054%29.png",
    wikiTitle:  "East\\\\u2013West Schism"
  },
  {
    id:         'T2-013',
    name:       "First Crusade \u2013 Siege of Jerusalem",
    year:       1099,
    yearMin:    1099,
    yearMax:    1099,
    lat:        31.7767,
    lng:        35.2345,
    loc:        "Jerusalem",
    img:        'T2-013_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "11th Century AD",
    c3:         "Jerusalem, Israel",
    desc:       "On July 15, 1099, after a five-week siege and a final assault beginning at dawn, the soldiers of the First Crusade breached the walls of Jerusalem and captured it after nearly five centuries of Muslim rule. The fall was accompanied by a massacre of the city's Muslim and Jewish inhabitants that shocked even some contemporary Christian chroniclers. The capture of Jerusalem was the defining military achievement of the Crusading movement and established four crusader states in the Levant that would survive for nearly two centuries before their eventual reconquest by Saladin.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/d/d2/Taking_of_Jerusalem_by_the_Crusaders%2C_15th_July_1099.jpg",
    wikiTitle:  "Siege of Jerusalem (1099)"
  },
  {
    id:         'T2-014',
    name:       "Battle of Austerlitz \u2013 Napoleon's Greatest Victory",
    year:       1805,
    lat:        49.0292,
    lng:        16.77,
    loc:        "Austerlitz (Slavkov), Czech Republic",
    img:        'T2-014_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "19th Century AD",
    c3:         "Austerlitz (Slavkov), Czech Republic, Czech Republic",
    desc:       "On December 2, 1805 \\\\u2014 the first anniversary of his coronation \\\\u2014 Napoleon Bonaparte won what he himself considered his greatest victory at Austerlitz, luring the combined Austrian and Russian emperors into attacking a seemingly weakened right flank before striking their exposed centre and dividing their army. The battle lasted less than nine hours; the Austro-Russian force lost 36,000 men and 180 guns against French losses of 9,000. The Treaty of Pressburg that followed dissolved the Holy Roman Empire \\\\u2014 an institution 844 years old \\\\u2014 and left Napoleon the undisputed master of continental Europe.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/b/ba/La_bataille_d%27Austerlitz._2_decembre_1805_%28Fran%C3%A7ois_G%C3%A9rard%29.jpg/3840px-La_bataille_d%27Austerlitz._2_decembre_1805_%28Fran%C3%A7ois_G%C3%A9rard%29.jpg",
    wikiTitle:  "Battle of Austerlitz"
  },
  {
    id:         'T2-015',
    name:       "Fall of Saigon \u2013 Vietnam War",
    year:       1975,
    lat:        10.7757,
    lng:        106.7011,
    loc:        "Presidential Palace / US Embassy, Saigon (Ho Chi Minh City)",
    img:        'T2-015_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "20th Century AD",
    c3:         "Presidential Palace / US Embassy, Saigon (Ho Chi Minh City), Vietnam",
    desc:       "On April 30, 1975, North Vietnamese tanks crashed through the gates of the Presidential Palace in Saigon, ending twenty years of war and unifying Vietnam under communist rule as the last American helicopters lifted off from the US Embassy roof with desperate evacuees clinging to the skids. The fall completed one of the most consequential military and political defeats in American history \\\\u2014 a conflict that had killed 58,000 Americans and between one and three million Vietnamese civilians. The iconic images of the evacuation became the defining symbols of American strategic overreach and the limits of military power.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/d/d9/Helicopter_evacuation_of_Saigon_atop_22_Gia_Long_Street.webp",
    wikiTitle:  "Fall of Saigon"
  },
  {
    id:         'T2-016',
    name:       "Hijra \u2013 Muhammad's Migration to Medina",
    year:       622,
    lat:        24.4672,
    lng:        39.6024,
    loc:        "Medina (Yathrib), Arabian Peninsula",
    img:        'T2-016_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "7th Century AD",
    c3:         "Medina (Yathrib), Arabian Peninsula, Saudi Arabia",
    desc:       "In 622 AD, the Prophet Muhammad and his small community of Muslim followers fled their home city of Mecca \\\\u2014 where the ruling Quraysh tribe had subjected them to persecution \\\\u2014 and migrated 340 kilometres north to the city of Yathrib, thereafter known as Medina. The Hijra is the foundational event of Islamic history: it marks year 1 of the Islamic calendar and the moment when a persecuted religious movement became a political community. Within a decade Muhammad had returned to Mecca in triumph; within a century, his successors had built an empire stretching from Spain to Central Asia.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b5/The_Prophet%27s_Mosque.jpg/640px-The_Prophet%27s_Mosque.jpg",
    wikiTitle:  "Hijra"
  },
  {
    id:         'T2-017',
    name:       "Mao Zedong Proclaims the People's Republic of China",
    year:       1949,
    lat:        39.9087,
    lng:        116.3975,
    loc:        "Tiananmen Gate, Beijing",
    img:        'T2-017_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "20th Century AD",
    c3:         "Tiananmen Gate, Beijing, China",
    desc:       "On October 1, 1949, Mao Zedong stood at the Tiananmen Gate in Beijing and proclaimed the founding of the People's Republic of China before a crowd of hundreds of thousands, ending the Chinese Civil War with a communist victory. The proclamation unified the world's most populous country under its first stable central government in over a century, following generations of imperial collapse, warlordism, Japanese occupation, and civil war. The People's Republic Mao founded would within three decades become the world's fastest-growing major economy and by the twenty-first century the primary geopolitical rival to the United States.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/8/85/Mao_Proclaiming_New_China.JPG",
    wikiTitle:  "Proclamation of the People's Republic of China"
  },
  {
    id:         'T2-018',
    name:       "Meiji Restoration \u2013 Japan Opens to the World",
    year:       1868,
    lat:        35.0235,
    lng:        135.768,
    loc:        "Kyoto Imperial Palace, Japan",
    img:        'T2-018_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "19th Century AD",
    c3:         "Kyoto Imperial Palace, Japan, Japan",
    desc:       "In 1868, a coalition of samurai overthrew the Tokugawa Shogunate \\\\u2014 which had ruled Japan in feudal isolation for 265 years \\\\u2014 and restored direct imperial rule under the teenage Emperor Meiji, triggering the most rapid and deliberate modernisation of any nation in history. Within forty years Japan had abolished feudalism, built railways, established a modern army and navy modelled on European lines, and drafted a constitution. The Meiji Restoration transformed Japan from a closed feudal state into the world's first non-Western industrial and imperial power.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/d/de/Emperor_Meiji_by_Takahashi_Yuichi.jpg",
    wikiTitle:  "Meiji Restoration"
  },
  {
    id:         'T2-019',
    name:       "Nanjing Massacre",
    year:       1937,
    yearMin:    1937,
    yearMax:    1938,
    lat:        32.0617,
    lng:        118.78,
    loc:        "Nanjing, China",
    img:        'T2-019_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "20th Century AD",
    c3:         "Nanjing, China, China",
    desc:       "Between December 1937 and January 1938, following the fall of China's capital Nanjing to the Imperial Japanese Army, Japanese troops carried out six weeks of systematic mass murder, rape, and looting that killed an estimated 200,000 to 300,000 Chinese civilians and prisoners of war. Japanese officers held killing competitions reported in their own press; the International Military Tribunal for the Far East later prosecuted commanding officers for crimes against humanity. The Nanjing Massacre remains the most painful episode in Sino-Japanese relations and a central reference point in Chinese national memory of the war.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/7/72/Nanking_bodies_1937.jpg",
    wikiTitle:  "Nanjing Massacre"
  },
  {
    id:         'T2-020',
    name:       "Spanish Armada \u2013 Battle off Gravelines",
    year:       1588,
    lat:        51.01,
    lng:        2.13,
    loc:        "Off Gravelines, English Channel, France/Belgium",
    img:        'T2-020_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "16th Century AD",
    c3:         "Off Gravelines, English Channel, France/Belgium, France/Belgium (sea)",
    desc:       "On August 8, 1588, English warships under Lord Howard and Francis Drake inflicted decisive damage on the Spanish Armada in a running battle off the Flemish coast near Gravelines, shattering Philip II of Spain's plan to transport his army to invade Protestant England. The battle culminated England's defensive strategy of harassing the Armada through the Channel, and subsequent storms that scattered the surviving ships around the British Isles destroyed half the fleet. The defeat ended Spanish naval supremacy and began a gradual shift in European power toward the northern maritime states.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/2/2e/Armadaroute.jpg",
    wikiTitle:  "Spanish Armada"
  },
  {
    id:         'T2-021',
    name:       "Hannibal Crosses the Alps",
    year:       -218,
    lat:        44.9,
    lng:        6.9,
    loc:        "Mont-Cenis or Clapier Pass, Alps",
    img:        'T2-021_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "3rd Century BC",
    c3:         "Mont-Cenis or Clapier Pass, Alps, France/Italy border",
    desc:       "In late 218 BC, the Carthaginian general Hannibal Barca led an army of approximately 38,000 infantry, 8,000 cavalry, and 37 war elephants across the Alps in a crossing of fifteen days through snow, ice, and mountain ambushes that has no parallel in ancient military history. Ancient sources disagree on the exact pass, and modern scholars still debate the route, but the physical achievement was extraordinary \\\\u2014 Hannibal lost nearly half his army to cold, hunger, and Gallic attacks before descending into Italy. The crossing achieved complete strategic surprise; Hannibal went on to win three devastating defeats against Rome at the Trebia, Lake Trasimene, and Cannae.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/4/48/Hannibal_route_of_invasion-en.svg/960px-Hannibal_route_of_invasion-en.svg.png",
    wikiTitle:  "Hannibal's crossing of the Alps"
  },
  {
    id:         'T2-022',
    name:       "Conquest of Tenochtitlan by Hern\u00e1n Cort\u00e9s",
    year:       1521,
    lat:        19.4326,
    lng:        -99.1332,
    loc:        "Tenochtitlan (Mexico City), Mexico",
    img:        'T2-022_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "16th Century AD",
    c3:         "Tenochtitlan (Mexico City), Mexico, Mexico",
    desc:       "On August 13, 1521, after a siege of 75 days, Spanish conquistador Hern\\\\u00e1n Cort\\\\u00e9s and his Tlaxcalan allies captured the Aztec capital of Tenochtitlan \\\\u2014 then one of the largest cities on earth \\\\u2014 ending the reign of the Aztec Triple Alliance and the last independent Mesoamerican empire. The city fell in street-by-street fighting after the emperor Cuauht\\\\u00e9moc was captured attempting to escape by canoe; Cort\\\\u00e9s had the city razed and built Mexico City on its ruins. The conquest, enabled by smallpox epidemics that had killed perhaps half the indigenous population, initiated three centuries of Spanish colonial rule in Mexico.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/2/20/Conquista-de-M%C3%A9xico-por-Cort%C3%A9s-Tenochtitlan-Painting.png",
    wikiTitle:  "Fall of Tenochtitlan"
  },
  {
    id:         'T2-023',
    name:       "Gandhi's Salt March",
    year:       1930,
    yearMin:    1930,
    yearMax:    1930,
    lat:        20.9042,
    lng:        72.6368,
    loc:        "Dandi Beach, Gujarat, India",
    img:        'T2-023_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "20th Century AD",
    c3:         "Dandi Beach, Gujarat, India, India",
    desc:       "On March 12, 1930, Mahatma Gandhi set out from his ashram at Sabarmati with 78 followers on a 241-mile march to the sea at Dandi, Gujarat, to protest the British salt tax \\\\u2014 a law that forbade Indians from collecting or selling salt. When Gandhi picked up a lump of natural salt from the beach on April 6 and urged Indians everywhere to do the same, he triggered the largest campaign of civil disobedience in Indian history. The Salt March demonstrated that nonviolent mass resistance could be a decisive political weapon and accelerated the movement that led to Britain's withdrawal from India in 1947.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/7/7c/Marche_sel.jpg",
    wikiTitle:  "Salt March"
  },
  {
    id:         'T2-024',
    name:       "Korean War \u2013 Inchon Landing",
    year:       1950,
    lat:        37.4563,
    lng:        126.6142,
    loc:        "Inchon (Incheon), South Korea",
    img:        'T2-024_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "20th Century AD",
    c3:         "Inchon (Incheon), South Korea, South Korea",
    desc:       "Between November 27 and 30, 1950, the Turkish Brigade \\\\u2014 5,000 soldiers deployed to Korea as part of the UN force \\\\u2014 fought a desperate rearguard action at Kunu-ri as the Chinese People's Volunteer Army launched its massive surprise offensive, allowing US and UN troops to escape encirclement at enormous cost to themselves. The Turkish soldiers fought in hand-to-hand combat with bayonets when they ran low on ammunition, suffering over 3,000 casualties \\\\u2014 nearly two-thirds of their total strength. The Battle of Kunu-ri remains a defining moment in Turkish military history and the foundation of the deep bond between Turkey and South Korea.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/5/56/Korean_War_T34_and_infantry.jpg/640px-Korean_War_T34_and_infantry.jpg",
    wikiTitle:  "Battle_of_Kunu-ri"
  },
  {
    id:         'T2-025',
    name:       "Qin Shi Huang Unifies China",
    year:       -221,
    lat:        34.3841,
    lng:        109.2785,
    loc:        "Xianyang (near Xi'an), Shaanxi, China",
    img:        'T2-025_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "3rd Century BC",
    c3:         "Xianyang (near Xi'an), Shaanxi, China, China",
    desc:       "In 221 BC, the king of the Qin state, Ying Zheng, completed the conquest of the last of the six rival Chinese kingdoms and proclaimed himself Qin Shi Huang \\u2014 First Emperor \\u2014 unifying China for the first time under a single centralised government after five centuries of fragmentation and warfare during the Warring States period. He immediately embarked on a programme of standardisation that transformed the country: one script, one system of weights and measures, one currency, one axle width for carts to ensure roads were universally usable, and one legal code. Though the Qin Dynasty itself lasted only fifteen years before collapsing under the weight of his brutal labour projects and his successors' incompetence, the unified Chinese state Qin Shi Huang created has persisted in one form or another for over two millennia.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/3/3f/QinShiHuang19century.jpg",
    wikiTitle:  "Qin Shi Huang"
  },
  {
    id:         'T2-026',
    name:       "Genghis Khan Unites the Mongol Tribes",
    year:       1206,
    lat:        48.9939,
    lng:        104.9621,
    loc:        "Khentii Mountains, Mongolia (Kurultai site)",
    img:        'T2-026_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "13th Century AD",
    c3:         "Khentii Mountains, Mongolia (Kurultai site), Mongolia",
    desc:       "In 1206 AD, a great assembly of Mongol tribal leaders gathered at the source of the Onon River in the Khentii Mountains and proclaimed Tem\\u00fcjin \\u2014 who had spent decades unifying the fractious Mongol tribes through a combination of military genius, brutal suppression of rivals, and a revolutionary policy of meritocratic promotion regardless of birth \\u2014 the Great Khan of all the Mongols, giving him the title Genghis Khan. Within two decades, the unified Mongol nation Genghis Khan created would launch the most explosive military expansion in history, ultimately building an empire stretching from the Pacific Ocean to Eastern Europe \\u2014 the largest contiguous land empire ever assembled. His yasa legal code, religious tolerance, and promotion of trade along the Silk Road reshaped the entire Eurasian world.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/3/35/YuanEmperorAlbumGenghisPortrait.jpg",
    wikiTitle:  "Genghis Khan"
  },
  {
    id:         'T2-027',
    name:       "Wright Brothers' First Powered Flight",
    year:       1903,
    lat:        36.0157,
    lng:        -75.6657,
    loc:        "Kill Devil Hills, Kitty Hawk, North Carolina",
    img:        'T2-027_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "20th Century AD",
    c3:         "Kill Devil Hills, Kitty Hawk, North Carolina, USA",
    desc:       "On December 17, 1903, at Kill Devil Hills near Kitty Hawk, North Carolina, Orville and Wilbur Wright achieved the first sustained, controlled, powered heavier-than-air flight in history \\u2014 a hop of 12 seconds and 37 metres in a biplane they had designed and built themselves in their Ohio bicycle shop, watched by five witnesses and a camera. By the fourth flight of the same day, Wilbur kept the Flyer aloft for 59 seconds and covered 260 metres. The achievement, which the brothers had pursued with systematic scientific rigour while contemporaries with far greater funding failed, compressed what had seemed like a generation of future technology into a single freezing morning, and within 66 years humans would reach the Moon.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/a/af/Orville_Wright_1905-crop.jpg",
    wikiTitle:  "Wright brothers"
  },
  {
    id:         'T2-028',
    name:       "Rise of Islam \u2013 Conquest of Mecca",
    year:       630,
    lat:        21.4225,
    lng:        39.8262,
    loc:        "Mecca, Arabian Peninsula",
    img:        'T2-028_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "7th Century AD",
    c3:         "Mecca, Arabian Peninsula, Saudi Arabia",
    desc:       "In January 630 AD, the Prophet Muhammad led an army of approximately 10,000 Muslim soldiers from Medina to Mecca \\u2014 the city that had expelled him eight years earlier \\u2014 and accepted its surrender with minimal bloodshed in what Islamic tradition records as a largely peaceful conquest. Muhammad declared a general amnesty for most of the city's population, entered the Kaaba, and ordered the destruction of the idols that had made Mecca the religious centre of pre-Islamic Arabia. The conquest of Mecca marked the definitive triumph of Islam in the Arabian Peninsula and removed the last significant political obstacle to the unification of Arabia under a single faith \\u2014 a unification that would within a decade propel Arab armies to conquer Persia, Egypt, and the Levant.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/5/50/Paris%2C_BnF%2C_Suppl%C3%A9ment_Persan_1030_fol._305v-306r_Muhammad_and_Ali_lead_destruction_of_Meccan_idols.jpg/3840px-Paris%2C_BnF%2C_Suppl%C3%A9ment_Persan_1030_fol._305v-306r_Muhammad_and_Ali_lead_destruction_of_Meccan_idols.jpg",
    wikiTitle:  "Conquest of Mecca"
  },
  {
    id:         'T2-029',
    name:       "Iranian Revolution \u2013 Fall of the Shah",
    year:       1979,
    lat:        35.6892,
    lng:        51.389,
    loc:        "Tehran, Iran",
    img:        'T2-029_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "20th Century AD",
    c3:         "Tehran, Iran, Iran",
    desc:       "On February 11, 1979, the Iranian Revolution reached its climax when the military declared neutrality and revolutionary forces overran the last government strongholds in Tehran, completing the overthrow of Shah Mohammad Reza Pahlavi \\u2014 who had already fled \\u2014 and ending 2,500 years of Iranian monarchy. The revolution, which had begun as a broad coalition of liberals, leftists, nationalists, and Islamists united only by opposition to the Shah, rapidly consolidated under the leadership of Ayatollah Khomeini, who returned from Paris exile to establish the world's first modern Islamic Republic. The revolution transformed Iran from a pro-Western monarchy into an anti-Western theocracy and altered the geopolitics of the Middle East irreversibly.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/4/4a/Mass_demonstration_in_Iran%2C_date_unknown.jpg",
    wikiTitle:  "Iranian Revolution"
  },
  {
    id:         'T2-030',
    name:       "Battle of Agincourt",
    year:       1415,
    lat:        50.46,
    lng:        2.14,
    loc:        "Agincourt (Azincourt), Pas-de-Calais, France",
    img:        'T2-030_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "15th Century AD",
    c3:         "Agincourt (Azincourt), Pas-de-Calais, France, France",
    desc:       "On October 25, 1415 \\u2014 St Crispin's Day \\u2014 an English army of approximately 6,000 exhausted, hungry, and dysentery-weakened soldiers under Henry V destroyed a French force of perhaps 12,000 to 36,000 at Agincourt in northern France, killing thousands of French knights and nobility in what became the most celebrated English military victory of the Hundred Years' War. The English longbowmen, positioned on a narrow front between two woods that neutralised the French numerical advantage, unleashed a storm of arrows that stopped the advancing French men-at-arms in the churned mud and cut them down. Shakespeare's Henry V made Agincourt the defining image of English martial heroism, and the speech he wrote for Henry on the eve of battle \\u2014 'We few, we happy few, we band of brothers' \\u2014 remains the most famous military oration in the English language.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/b/be/Schlacht_von_Azincourt.jpg",
    wikiTitle:  "Battle of Agincourt"
  },
  {
    id:         'T2-031',
    name:       "American Declaration of Independence",
    year:       1776,
    lat:        39.9489,
    lng:        -75.15,
    loc:        "Pennsylvania State House (Independence Hall), Philadelphia",
    img:        'T2-031_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "18th Century AD",
    c3:         "Pennsylvania State House (Independence Hall), Philadelphia, USA",
    desc:       "On August 2, 1776 \\u2014 weeks after the Continental Congress voted for independence on July 2 and approved the text on July 4 \\u2014 most of the 56 delegates to the Continental Congress signed the Declaration of Independence at the Pennsylvania State House in Philadelphia, formally announcing the separation of thirteen British colonies from the Crown on the grounds that all men are created equal and governments derive their just powers from the consent of the governed. Thomas Jefferson's declaration, edited by Franklin and Adams before adoption, drew on Enlightenment philosophy to articulate a theory of government by popular consent that had never before been the founding document of a state. The Declaration became the most influential political text in modern history, inspiring democratic revolutions on every continent.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/8/8f/United_States_Declaration_of_Independence.jpg/3840px-United_States_Declaration_of_Independence.jpg",
    wikiTitle:  "United States Declaration of Independence"
  },
  {
    id:         'T2-032',
    name:       "Mansa Musa's Pilgrimage to Mecca",
    year:       1324,
    yearMin:    1324,
    yearMax:    1325,
    lat:        30.0626,
    lng:        31.2497,
    loc:        "Cairo, Egypt (en route to Mecca)",
    img:        'T2-032_img1.png',
    yaw:        0,
    c1:         "Africa",
    c2:         "14th Century AD",
    c3:         "Cairo, Egypt (en route to Mecca), Egypt",
    desc:       "In 1324, Mansa Musa \\u2014 emperor of the Mali Empire and the wealthiest individual in recorded history, controlling over half the world's gold supply \\u2014 departed his capital Niani on a pilgrimage to Mecca with a retinue estimated at 60,000 people, including 12,000 enslaved servants and 500 heralds each carrying a golden staff, and 80 to 100 camels each loaded with 136 kilograms of gold dust. His procession through Cairo was so extravagant \\u2014 giving away gold to subjects and the poor and buying luxury goods \\u2014 that he reportedly caused significant gold inflation in Egypt and the wider Mediterranean region for more than a decade. His pilgrimage placed the Mali Empire on European maps for the first time and announced to the world that sub-Saharan Africa contained a civilisation of extraordinary wealth and sophistication.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/4/46/Catalan_Atlas_BNF_Sheet_6_Mansa_Musa_%28cropped%29.jpg",
    wikiTitle:  "Mansa Musa"
  },
  {
    id:         'T2-033',
    name:       "Tiananmen Square Protests",
    year:       1989,
    yearMin:    1989,
    yearMax:    1989,
    lat:        39.9042,
    lng:        116.3912,
    loc:        "Tiananmen Square, Beijing",
    img:        'T2-033_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "20th Century AD",
    c3:         "Tiananmen Square, Beijing, China",
    desc:       "In the spring of 1989, hundreds of thousands of Chinese students and workers gathered in Tiananmen Square in Beijing in a pro-democracy demonstration inspired by the death of the reformist leader Hu Yaobang, demanding political liberalisation, press freedom, and an end to corruption in a protest that at its peak drew one million people and spread to over 400 Chinese cities. On the night of June 3\\u20134, the Chinese government declared martial law and deployed the military with tanks and assault rifles; troops fired on protesters and civilians as they cleared the square and surrounding streets. The precise death toll remains unknown and suppressed by the Chinese government \\u2014 estimates range from several hundred to several thousand \\u2014 and the image of a lone man standing before a column of tanks became the defining photograph of individual resistance to state power in the twentieth century.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/e/e7/Ud%C3%A1losti_na_n%C3%A1m%C4%9Bst%C3%AD_Tian_an_men%2C_%C4%8C%C3%ADna_1989%2C_foto_Ji%C5%99%C3%AD_Tondl.jpg",
    wikiTitle:  "1989 Tiananmen Square protests and massacre"
  },
  {
    id:         'T2-034',
    name:       "Korean War Armistice",
    year:       1953,
    lat:        37.9534,
    lng:        126.6614,
    loc:        "Panmunjom, Korean DMZ",
    img:        'T2-034_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "20th Century AD",
    c3:         "Panmunjom, Korean DMZ, North Korea/South Korea border",
    desc:       "On July 27, 1953, representatives of the United Nations Command, the Korean People's Army, and the Chinese People's Volunteer Army signed the Korean Armistice Agreement at Panmunjom, ending three years of fighting that had killed approximately 5 million people \\u2014 soldiers and civilians \\u2014 and left the Korean Peninsula divided at roughly the same line where the war had begun. No peace treaty was ever signed: the armistice created a ceasefire and the Demilitarized Zone, which remains the most heavily fortified border on Earth. The Korean War, often called the Forgotten War between the better-remembered Second World War and Vietnam, established the precedent of limited conventional warfare between nuclear-armed powers that defined Cold War military strategy for four decades.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/b/b5/Korean_War_armistice_agreement_1953.jpg",
    wikiTitle:  "Korean Armistice Agreement"
  },
  {
    id:         'T2-035',
    name:       "Construction of the Taj Mahal",
    year:       1632,
    yearMin:    1632,
    yearMax:    1653,
    lat:        27.1751,
    lng:        78.0421,
    loc:        "Agra, Uttar Pradesh, India",
    img:        'T2-035_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "17th Century AD",
    c3:         "Agra, Uttar Pradesh, India, India",
    desc:       "Between 1632 and 1653, the Mughal Emperor Shah Jahan oversaw the construction of the Taj Mahal at Agra on the banks of the Yamuna River \\u2014 a mausoleum for his beloved wife Mumtaz Mahal who had died in childbirth \\u2014 employing approximately 20,000 workers and artisans from across the Islamic world and importing white marble from Rajasthan and precious stones from as far as China, Afghanistan, and Sri Lanka. The complex, designed by the Persian architect Ustad Ahmad Lahauri, achieves an extraordinary optical illusion: the main dome appears to maintain the same size as you approach it, and the four minarets lean slightly outward so that in an earthquake they would fall away from the central tomb. The Taj Mahal is consistently ranked the greatest work of Mughal architecture and one of the most beautiful buildings ever constructed.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/1/1d/Taj_Mahal_%28Edited%29.jpeg",
    wikiTitle:  "Taj Mahal"
  },
  {
    id:         'T2-036',
    name:       "WWI \u2013 Armistice Day",
    year:       1918,
    lat:        49.4167,
    lng:        3.0833,
    loc:        "Compi\u00e8gne Forest, France (Armistice railway carriage)",
    img:        'T2-036_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Compi\u00e8gne Forest, France (Armistice railway carriage), France",
    desc:       "At the eleventh hour of the eleventh day of the eleventh month of 1918, the guns fell silent along the Western Front as the Armistice between the Allied Powers and Germany came into effect, ending four years and three months of the most destructive war in human history to that point \\u2014 a conflict that had killed an estimated 20 million people and wounded 21 million more. The armistice had been signed at 5am in a railway carriage in the Compi\\u00e8gne Forest, but fighting continued until the designated 11am ceasefire, with thousands more men killed in the final hours by commanders unwilling to let the war end quietly. The eleventh of November became the most solemnly observed date in the calendars of Britain, France, and the Commonwealth, marked every year by two minutes of silence at exactly 11am.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/2/2f/Armisticetrain_%28slight_crop%29.jpg",
    wikiTitle:  "Armistice of 11 November 1918"
  },
  {
    id:         'T2-037',
    name:       "Michelangelo Paints the Sistine Chapel Ceiling",
    year:       1508,
    yearMin:    1508,
    yearMax:    1512,
    lat:        41.9029,
    lng:        12.4545,
    loc:        "Sistine Chapel, Vatican City",
    img:        'T2-037_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "16th Century AD",
    c3:         "Sistine Chapel, Vatican City, Italy (Vatican)",
    desc:       "Between 1508 and 1512, Michelangelo Buonarroti \u2014 who considered himself primarily a sculptor and had initially refused the commission \u2014 painted the ceiling of the Sistine Chapel in the Vatican under Pope Julius II, covering 500 square metres with over 300 figures depicting nine scenes from Genesis, surrounded by prophets, sibyls, and ancestors of Christ, in what is widely regarded as the greatest single work of art in Western history. Working lying on his back on specially constructed scaffolding (not on a ladder, as popular myth holds), Michelangelo invented new techniques of fresco painting to achieve the ceiling's extraordinary illusionistic depth and the muscular, superhuman figures \u2014 especially the famous Creation of Adam \u2014 that became the defining image of the High Renaissance. The ceiling was cleaned and restored between 1980 and 1994, revealing colours far more vivid than four centuries of candle smoke had suggested.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/5/5b/Michelangelo_-_Creation_of_Adam_%28cropped%29.jpg/640px-Michelangelo_-_Creation_of_Adam_%28cropped%29.jpg",
    wikiTitle:  "Sistine Chapel ceiling"
  },
  {
    id:         'T3-001',
    name:       "Seneca Falls Convention",
    year:       1848,
    lat:        42.9107,
    lng:        -76.7968,
    loc:        "Seneca Falls, New York State",
    img:        'T3-001_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "19th Century AD",
    c3:         "Seneca Falls, New York State, USA",
    desc:       "On July 19\\\\u201320, 1848, some three hundred women and men gathered at the Wesleyan Methodist Chapel in Seneca Falls, New York, for the first women's rights convention in American history, organised by Elizabeth Cady Stanton and Lucretia Mott. The convention produced the Declaration of Sentiments \\\\u2014 modelled deliberately on the Declaration of Independence \\\\u2014 asserting that all men and women are created equal and demanding, most controversially, the right of women to vote. The Seneca Falls Convention launched the American suffrage movement; 72 years later, on August 26, 1920, the Nineteenth Amendment finally guaranteed American women the right to vote.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/4/43/Woman%27s_Rights_Convention.jpg",
    wikiTitle:  "Seneca Falls Convention"
  },
  {
    id:         'T3-002',
    name:       "Daily Life in Ancient Uruk, Mesopotamia",
    year:       -3500,
    yearMin:    -3500,
    yearMax:    -3000,
    lat:        33.2232,
    lng:        43.6793,
    loc:        "Mesopotamia (modern central Iraq)",
    img:        'T3-002_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "35th Century BC",
    c3:         "Mesopotamia (modern central Iraq), Iraq",
    desc:       "Between approximately 3500 and 3000 BC, the city of Uruk in Mesopotamia \\\\u2014 in what is now central Iraq \\\\u2014 became the world's first urban civilisation, home to an estimated 40,000 to 80,000 people and the birthplace of writing, monumental architecture, and long-distance trade. The city's temple complexes required administrative systems sophisticated enough to track tribute, rations, and labour \\\\u2014 generating the world's first written records in the form of clay tablets inscribed with cuneiform pictographs. Uruk's innovations in writing, accounting, and urban organisation spread throughout the ancient Near East and laid the foundations of all subsequent literate civilisation.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a5/Tableta_con_trillo%2C_Jemdet_Nasr%2C_Iraq.jpg/640px-Tableta_con_trillo%2C_Jemdet_Nasr%2C_Iraq.jpg",
    wikiTitle:  "Uruk"
  },
  {
    id:         'T3-003',
    name:       "Council of Nicaea \u2013 First Ecumenical Council",
    year:       325,
    lat:        40.4272,
    lng:        29.7233,
    loc:        "Nicaea (modern \u0130znik), Turkey",
    img:        'T3-003_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "4th Century AD",
    c3:         "Nicaea (modern \u0130znik), Turkey, Turkey",
    desc:       "In May 325 AD, the Emperor Constantine I summoned approximately 300 bishops from across the Christian world to the city of Nicaea in Anatolia for the first ecumenical council in Christian history, convened to resolve the Arian controversy \\\\u2014 whether Jesus Christ was of the same substance as God the Father or merely similar. The council declared Arianism a heresy, formulated the Nicene Creed that defined orthodox Trinitarian Christianity, and established the date of Easter. By summoning and presiding over the council, Constantine established a precedent of imperial involvement in church affairs that defined the relationship between Christianity and political power across the medieval world.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/1/1d/%CE%9C%CE%B9%CF%87%CE%B1%CE%AE%CE%BB_%CE%94%CE%B1%CE%BC%CE%B1%CF%83%CE%BA%CE%B7%CE%BD%CF%8C%CF%82_-_%CE%91%CE%84_%CE%9F%CE%B9%CE%BA%CE%BF%CF%85%CE%BC%CE%B5%CE%BD%CE%B9%CE%BA%CE%AE_%CE%A3%CF%8D%CE%BD%CE%BF%CE%B4%CE%BF%CF%82_7741.jpg",
    wikiTitle:  "First Council of Nicaea"
  },
  {
    id:         'T3-004',
    name:       "Battle of Adwa \u2013 Ethiopia Defeats Italy",
    year:       1896,
    lat:        14.1667,
    lng:        38.9,
    loc:        "Adwa, Tigray, Ethiopia",
    img:        'T3-004_img1.png',
    yaw:        0,
    c1:         "Africa",
    c2:         "19th Century AD",
    c3:         "Adwa, Tigray, Ethiopia, Ethiopia",
    desc:       "On March 1, 1896, Ethiopian Emperor Menelik II's army of approximately 100,000 soldiers decisively defeated an invading Italian force of 17,000 at the Battle of Adwa \\\\u2014 the most significant defeat of a European colonial army by an African force during the scramble for Africa. Italy had assumed its modern weapons and military discipline would easily overcome a kingdom it considered primitive; instead, Ethiopian forces armed with modern rifles outmanoeuvred and annihilated three separate Italian columns simultaneously. The victory preserved Ethiopian independence, making it the only African nation never colonised in the nineteenth century, and inspired anti-colonial movements across Africa for generations.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/3/33/Adoua_1.jpg",
    wikiTitle:  "Battle of Adwa"
  },
  {
    id:         'T3-005',
    name:       "Siege of Masada",
    year:       73,
    yearMin:    73,
    yearMax:    74,
    lat:        31.3156,
    lng:        35.3536,
    loc:        "Masada fortress, Judean Desert",
    img:        'T3-005_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "1st Century AD",
    c3:         "Masada fortress, Judean Desert, Israel",
    desc:       "In 73\\\\u201374 AD, the Roman Tenth Legion besieged the fortress of Masada atop an isolated plateau above the Dead Sea \\\\u2014 the last holdout of Jewish Zealots following the destruction of Jerusalem and the Second Temple. After months of siege, the Romans completed a massive earthen ramp to the summit and breached the walls; according to the historian Josephus, the 960 defenders chose collective suicide over capture. Masada \\\\u2014 'Masada shall not fall again' \\\\u2014 became a foundational symbol of Jewish resistance and national identity in the modern State of Israel.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1b/Masada_aerial_2.jpg/640px-Masada_aerial_2.jpg",
    wikiTitle:  "Masada"
  },
  {
    id:         'T3-006',
    name:       "Destruction of Carthage \u2013 Third Punic War",
    year:       -146,
    lat:        36.8528,
    lng:        10.3233,
    loc:        "Carthage, near Tunis, Tunisia",
    img:        'T3-006_img1.png',
    yaw:        0,
    c1:         "Africa",
    c2:         "2nd Century BC",
    c3:         "Carthage, near Tunis, Tunisia, Tunisia",
    desc:       "In 146 BC, following the Third Punic War, the Roman general Scipio Aemilianus razed Carthage \\\\u2014 Rome's great rival for Mediterranean dominance for three centuries \\\\u2014 to its foundations after a brutal street-by-street battle, enslaving 50,000 survivors. The Roman Senate had debated for years whether to destroy or spare the city; the senator Cato had ended every speech with 'Carthage must be destroyed.' The destruction of Carthage left Rome without a serious rival in the western Mediterranean and set it on the path to the imperial dominance that would shape Western civilisation.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/1/16/City_of_Carthage_circa_149_BC.png",
    wikiTitle:  "Third Punic War"
  },
  {
    id:         'T3-007',
    name:       "Partition of India",
    year:       1947,
    lat:        31.604,
    lng:        74.5284,
    loc:        "Wagah, Punjab (India-Pakistan border)",
    img:        'T3-007_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "20th Century AD",
    c3:         "Wagah, Punjab (India-Pakistan border), India/Pakistan",
    desc:       "On August 14\\\\u201315, 1947, the British Indian Empire was partitioned into two independent nations \\\\u2014 the secular Republic of India and the Muslim state of Pakistan \\\\u2014 in one of the largest and most violent mass migrations in human history, displacing approximately 14 million people and killing between 200,000 and 2 million in intercommunal violence. The partition, announced with inadequate planning by the last Viceroy Lord Mountbatten, drew a border through the provinces of Punjab and Bengal that divided communities and families that had coexisted for centuries. The trauma of partition continues to shape the political relationship between India and Pakistan, two nuclear-armed neighbours who have fought three wars since independence.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/4/47/Brit_IndianEmpireReligions3.jpg",
    wikiTitle:  "Partition of India"
  },
  {
    id:         'T3-008',
    name:       "Foundation of Rome",
    year:       -753,
    yearMin:    -753,
    yearMax:    -625,
    lat:        41.8902,
    lng:        12.49,
    loc:        "Palatine Hill, Rome",
    img:        'T3-008_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "8th Century BC",
    c3:         "Palatine Hill, Rome, Italy",
    desc:       "According to Roman tradition, the city of Rome was founded on April 21, 753 BC, by the twin brothers Romulus and Remus \\\\u2014 sons of the war god Mars \\\\u2014 who had been suckled as infants by a she-wolf; Romulus killed his brother in a dispute over the city's walls and became its first king. Archaeological evidence from the Palatine Hill confirms significant settlement from the mid-eighth century BC, broadly consistent with the traditional foundation date. From this hilltop settlement above the Tiber, Rome grew over eight centuries to become an empire of 60 million people stretching from Scotland to Mesopotamia \\\\u2014 the most influential political entity in Western history.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/e/ea/Kapitolinische_W%C3%B6lfin_Museum_Capitolini.jpg/3840px-Kapitolinische_W%C3%B6lfin_Museum_Capitolini.jpg",
    wikiTitle:  "Founding of Rome"
  },
  {
    id:         'T3-009',
    name:       "Great Pyramid of Giza \u2013 Construction",
    year:       -2600,
    yearMin:    -2600,
    yearMax:    -2500,
    lat:        29.9792,
    lng:        31.1342,
    loc:        "Giza Plateau, Egypt",
    img:        'T3-009_img1.png',
    yaw:        0,
    c1:         "Africa",
    c2:         "26th Century BC",
    c3:         "Giza Plateau, Egypt, Egypt",
    desc:       "Around 2560 BC, the Fourth Dynasty Pharaoh Khufu oversaw the construction of the Great Pyramid of Giza \\\\u2014 originally 146 metres tall and composed of an estimated 2.3 million stone blocks weighing between 2.5 and 15 tonnes each \\\\u2014 in what remains the most ambitious construction project in human history. Archaeological evidence shows that the pyramid was built not by slaves but by approximately 20,000 to 30,000 skilled and semi-skilled workers who were fed, housed, and given medical care by the state. The Great Pyramid remained the tallest structure on Earth for 3,800 years, until the completion of Lincoln Cathedral in England around 1311 AD.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/e/e7/Great_Pyramid_of_Giza_-_Pyramid_of_Khufu.jpg",
    wikiTitle:  "Great Pyramid of Giza"
  },
  {
    id:         'T3-010',
    name:       "Siege of Troy",
    year:       -1260,
    yearMin:    -1260,
    yearMax:    -1180,
    lat:        39.9572,
    lng:        26.2387,
    loc:        "Hisarlik, \u00c7anakkale, Turkey",
    img:        'T3-010_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "13th Century BC",
    c3:         "Hisarlik, \u00c7anakkale, Turkey, Turkey",
    desc:       "The Trojan War, dated by ancient Greeks to approximately 1180 BC, was a ten-year conflict between an alliance of Greek city-states and the wealthy city of Troy on the Anatolian coast, triggered \\\\u2014 according to the Homeric tradition \\\\u2014 by the Trojan prince Paris's abduction of Helen, queen of Sparta. Archaeological excavations at Hisarlik in modern Turkey have confirmed the existence of a Bronze Age city that was violently destroyed around 1180 BC, suggesting a historical kernel beneath Homer's legendary account. The Iliad and the Odyssey became the founding works of Western literature and established the narrative archetypes \\\\u2014 the hero's journey, the soldier's homecoming \\\\u2014 that storytellers have returned to ever since.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/b/ba/Akhilleus_Patroklos_Antikensammlung_Berlin_F2278.jpg",
    wikiTitle:  "Trojan War"
  },
  {
    id:         'T3-011',
    name:       "School of Athens \u2013 Socrates Teaching in the Agora",
    year:       -430,
    yearMin:    -430,
    yearMax:    -399,
    lat:        37.9755,
    lng:        23.7235,
    loc:        "Ancient Agora of Athens, Greece",
    img:        'T3-011_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "5th Century BC",
    c3:         "Ancient Agora of Athens, Greece, Greece",
    desc:       "In the bustling Agora of Athens during the height of the city's golden age \\\\u2014 roughly 430 to 399 BC \\\\u2014 Socrates could be found daily engaging anyone willing to talk: merchants, politicians, generals, craftsmen, and young men hungry for wisdom, all subjected to his relentless questioning that exposed the gap between what people believed they knew and what they could actually justify. Unlike the Sophists who charged fees and claimed to teach virtue, Socrates insisted he knew nothing and merely asked questions \\\\u2014 the method his student Plato later called elenchus, or cross-examination. The Agora was not merely a marketplace but the civic heart of Athenian democracy, and Socrates made it his classroom for thirty years, gathering around him a circle of devoted students including Plato and Xenophon whose accounts became the founding texts of Western philosophy.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/a/a4/Socrates_Louvre.jpg",
    wikiTitle:  "Socrates"
  },
  {
    id:         'T3-012',
    name:       "Trial of Socrates",
    year:       -399,
    lat:        37.974,
    lng:        23.726,
    loc:        "Court of the Heliaia, Athens",
    img:        'T3-012_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "4th Century BC",
    c3:         "Court of the Heliaia, Athens, Greece",
    desc:       "In 399 BC, the city of Athens put its most famous philosopher on trial before a jury of 500 citizens on charges of impiety and corrupting the youth \\\\u2014 charges that were really a proxy for his decades of publicly embarrassing prominent Athenians by exposing the ignorance beneath their claimed wisdom. Socrates conducted his own defence in a manner that managed to antagonise the jury, refusing to propose exile and suggesting instead that Athens should reward him with free meals for life. He was condemned to death and executed by drinking hemlock \\\\u2014 providing Western philosophy with its foundational martyrdom and raising questions about democracy, free speech, and the individual versus the state that remain unresolved twenty-four centuries later.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/8/8c/David_-_The_Death_of_Socrates.jpg/640px-David_-_The_Death_of_Socrates.jpg",
    wikiTitle:  "Trial_of_Socrates"
  },
  {
    id:         'T3-013',
    name:       "Viking Raid on Lindisfarne",
    year:       793,
    lat:        55.6814,
    lng:        -1.7993,
    loc:        "Holy Island (Lindisfarne), Northumberland",
    img:        'T3-013_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "8th Century AD",
    c3:         "Holy Island (Lindisfarne), Northumberland, United Kingdom",
    desc:       "On June 8, 793 AD, Norse raiders arrived without warning at the monastery of Lindisfarne \\\\u2014 the Holy Island off the Northumbrian coast, one of the most sacred sites in the British Isles \\\\u2014 killing monks, throwing others into the sea, and carrying away gold, silver, and prisoners. The attack shocked the Christian world: the scholar Alcuin, writing from Charlemagne's court, declared it a punishment from God. The Lindisfarne raid is traditionally dated as the opening event of the Viking Age, which would transform the political geography of Europe over the next 250 years.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/6/6d/T.homas_Girtin_Lindisfarne_1798.jpg",
    wikiTitle:  "Sack of Lindisfarne"
  },
  {
    id:         'T3-014',
    name:       "Construction of the Great Wall of China \u2013 Qin Dynasty",
    year:       -221,
    yearMin:    -221,
    yearMax:    -206,
    lat:        40.4319,
    lng:        116.5704,
    loc:        "Badaling area, northern China",
    img:        'T3-014_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "3rd Century BC",
    c3:         "Badaling area, northern China, China",
    desc:       "Between approximately 221 and 206 BC, the First Emperor of China, Qin Shi Huang, ordered the connection and extension of existing northern frontier walls into a unified defensive system \\\\u2014 mobilising hundreds of thousands of soldiers, convicts, and labourers, many of whom died and were buried within the walls themselves. The original Qin walls have largely vanished; the iconic stone-and-brick Great Wall familiar from photographs was built primarily during the Ming Dynasty (1368\\\\u20131644 AD). Together the successive walls represent the largest construction project in human history and the most visible man-made structure from low Earth orbit.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/2/23/The_Great_Wall_of_China_at_Jinshanling-edit.jpg/3840px-The_Great_Wall_of_China_at_Jinshanling-edit.jpg",
    wikiTitle:  "Great Wall of China"
  },
  {
    id:         'T3-015',
    name:       "Battle of Sekigahara",
    year:       1600,
    lat:        35.3719,
    lng:        136.4709,
    loc:        "Sekigahara, Gifu Prefecture, Japan",
    img:        'T3-015_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "16th Century AD",
    c3:         "Sekigahara, Gifu Prefecture, Japan, Japan",
    desc:       "On October 21, 1600, two great coalitions of Japanese warlords clashed at Sekigahara in Gifu Prefecture in a battle that lasted a single day but decided the future of Japan for 265 years. Tokugawa Ieyasu's eastern coalition defeated the western forces loyal to the young Toyotomi heir, in part because several western lords switched sides at the critical moment. Ieyasu was appointed Shogun by the Emperor three years later, founding the Tokugawa Shogunate that would rule Japan in enforced isolation, peace, and rigid social hierarchy until the Meiji Restoration of 1868.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/9/9f/Sekigaharascreen.jpg",
    wikiTitle:  "Battle of Sekigahara"
  },
  {
    id:         'T3-016',
    name:       "Construction of Angkor Wat",
    year:       1113,
    yearMin:    1113,
    yearMax:    1150,
    lat:        13.4125,
    lng:        103.867,
    loc:        "Angkor, Siem Reap, Cambodia",
    img:        'T3-016_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "12th Century AD",
    c3:         "Angkor, Siem Reap, Cambodia, Cambodia",
    desc:       "Between approximately 1113 and 1150 AD, the Khmer Emperor Suryavarman II oversaw the construction of Angkor Wat \\\\u2014 a temple mountain covering 162 hectares dedicated to the Hindu god Vishnu \\\\u2014 that remains the largest religious structure ever built, surrounded by a moat 190 metres wide. The temple's five towers represent the peaks of Mount Meru, the home of the gods in Hindu cosmology, while its walls are covered in over half a kilometre of bas-relief carvings depicting Hindu mythology and historical battles. Angkor Wat is the only temple in the region never abandoned, converting from Hinduism to Buddhism in the 13th century, and today appears on the national flag of Cambodia.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/f/f5/Buddhist_monks_in_front_of_the_Angkor_Wat.jpg",
    wikiTitle:  "Angkor Wat"
  },
  {
    id:         'T3-017',
    name:       "Spartacus Slave Revolt",
    year:       -73,
    yearMin:    -73,
    yearMax:    -71,
    lat:        40.916,
    lng:        15.537,
    loc:        "Capua, Campania, Italy",
    img:        'T3-017_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "1st Century BC",
    c3:         "Capua, Campania, Italy, Italy",
    desc:       "In the summer of 73 BC, the gladiator Spartacus and his rebel followers \\\\u2014 trapped on the summit of Mount Vesuvius by the Roman praetor Glaber, who had blockaded the single path down \\\\u2014 braided ropes from the wild grapevines growing on the volcanic slopes and rappelled down the unguarded cliff face in a manoeuvre that completely surprised and routed the Roman force waiting below. Plutarch records this escape in specific detail, and Appian confirms the location: Vesuvius before it had become the volcano of Pompeii, still densely vegetated, its sheer cliffs providing both the trap and the unlikely means of escape. The breakout transformed what Rome had dismissed as a minor slave disturbance into a full rebellion that would last three years and require the full military resources of Crassus and Pompey to finally suppress.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/a/a2/%CE%A3%CF%80%CE%AC%CF%81%CF%84%CE%B1%CE%BA%CE%BF%CF%82_Spartacus%2CEspartaco_marble_statue_in_Louvre_%22%CE%9C%CE%B1%CE%B9%CE%B4%CE%BF%CE%B9_clan_-Meadow%22_slave_macedon-greek.jpg",
    wikiTitle:  "Spartacus"
  },
  {
    id:         'T3-018',
    name:       "Inauguration of the Roman Colosseum",
    year:       80,
    lat:        41.8902,
    lng:        12.4922,
    loc:        "Colosseum (Flavian Amphitheatre), Rome",
    img:        'T3-018_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "1st Century AD",
    c3:         "Colosseum (Flavian Amphitheatre), Rome, Italy",
    desc:       "In 80 AD, the Emperor Titus opened the Flavian Amphitheatre \\\\u2014 known to posterity as the Colosseum \\\\u2014 with one hundred days of games in which thousands of gladiators and animals fought and died before an audience of up to 80,000 spectators, making it the largest entertainment venue ever built in the ancient world. The structure, constructed largely by Jewish slaves following the sack of Jerusalem in 70 AD, rose four storeys and introduced architectural innovations \\\\u2014 including the velarium sunshade and numbered entrance arches \\\\u2014 that influenced stadium design for two millennia. The Colosseum hosted gladiatorial combat, animal hunts, and public executions, and stood as the symbol of Rome's capacity for both spectacular engineering and spectacular violence.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/d/de/Colosseo_2020.jpg/3840px-Colosseo_2020.jpg",
    wikiTitle:  "Colosseum"
  },
  {
    id:         'T3-019',
    name:       "Construction of Machu Picchu",
    year:       1440,
    yearMin:    1440,
    yearMax:    1460,
    lat:        -13.1631,
    lng:        -72.545,
    loc:        "Machu Picchu, Cusco Region, Peru",
    img:        'T3-019_img1.png',
    yaw:        0,
    c1:         "South America",
    c2:         "15th Century AD",
    c3:         "Machu Picchu, Cusco Region, Peru, Peru",
    desc:       "Around 1450 AD, the Inca Emperor Pachacuti ordered the construction of Machu Picchu on a ridge 2,430 metres above sea level in the Peruvian Andes \\\\u2014 a royal estate and religious sanctuary whose engineering achievement is unambiguous: 200 structures built from precisely fitted stone blocks without mortar on an almost inaccessible mountain peak. The site was abandoned around 1572 following the Spanish conquest and remained unknown to the outside world until the American historian Hiram Bingham was led there by a local farmer in 1911. Today Machu Picchu is the most visited archaeological site in the Americas and a defining symbol of pre-Columbian civilisation.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/e/eb/Machu_Picchu%2C_Peru.jpg",
    wikiTitle:  "Machu_Picchu"
  },
  {
    id:         'T3-020',
    name:       "Marco Polo Arrives at the Court of Kublai Khan",
    year:       1275,
    lat:        41.902,
    lng:        116.41,
    loc:        "Xanadu (Shangdu), Inner Mongolia, China",
    img:        'T3-020_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "13th Century AD",
    c3:         "Xanadu (Shangdu), Inner Mongolia, China, China (Inner Mongolia)",
    desc:       "In 1275 AD, the young Venetian merchant Marco Polo arrived at the summer capital of Kublai Khan \\\\u2014 the Mongol Emperor of China \\\\u2014 beginning a seventeen-year residence at the most powerful court on earth that would produce one of the most influential travel accounts ever written. Marco Polo served as an envoy and administrator for Kublai Khan across China, observing paper money, coal, postal relay systems, and cities of a scale that Europeans could not imagine. His account, The Travels of Marco Polo \\\\u2014 dictated to a romance writer while both were imprisoned in Genoa \\\\u2014 was widely disbelieved during his lifetime but inspired Columbus's westward search for Asia two centuries later.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/1/1b/YuanEmperorAlbumKhubilaiPortrait.jpg",
    wikiTitle:  "Kublai Khan"
  },
  {
    id:         'T3-021',
    name:       "Plague of Athens",
    year:       -430,
    yearMin:    -430,
    yearMax:    -426,
    lat:        37.965,
    lng:        23.72,
    loc:        "Piraeus and Athens, Greece",
    img:        'T3-021_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "5th Century BC",
    c3:         "Piraeus and Athens, Greece, Greece",
    desc:       "Beginning in 430 BC, during the second year of the Peloponnesian War while Athens was crowded with refugees sheltering behind its walls from Spartan invasion, a devastating epidemic swept through the city and killed approximately one-quarter to one-third of the Athenian population over four years, including the statesman Pericles. The disease, described in precise clinical detail by the historian Thucydides who survived it, caused such rapid social breakdown that people stopped observing religious and social norms, believing they would die before facing consequences. The Plague of Athens accelerated Athenian decline in the Peloponnesian War and raised fundamental questions about fortune and divine will that shaped Greek philosophical thought for generations.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/6/66/Plague_in_an_Ancient_City_LACMA_AC1997.10.1_%281_of_2%29.jpg",
    wikiTitle:  "Plague of Athens"
  },
  {
    id:         'T3-022',
    name:       "Library of Alexandria at Its Height",
    year:       -280,
    yearMin:    -280,
    yearMax:    -200,
    lat:        31.2156,
    lng:        29.9553,
    loc:        "Alexandria, Egypt",
    img:        'T3-022_img1.png',
    yaw:        0,
    c1:         "Africa",
    c2:         "3rd Century BC",
    c3:         "Alexandria, Egypt, Egypt",
    desc:       "At its height around 250 BC, the Library of Alexandria housed an estimated 400,000 to 700,000 papyrus scrolls covering every subject from mathematics and astronomy to drama and medicine, assembled through a deliberate policy of copying every book that passed through Alexandria's harbour. The Library was paired with the Mouseion, a research institution where scholars including Euclid, Archimedes, Eratosthenes \\\\u2014 who calculated the Earth's circumference within 1% accuracy \\\\u2014 and Aristarchus \\\\u2014 who proposed that the Earth orbits the Sun \\\\u2014 lived and worked. The Library was not destroyed in a single dramatic fire but declined gradually through neglect and political upheaval, representing the ancient world's most ambitious attempt to gather all human knowledge in one place.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/6/64/Ancientlibraryalex.jpg",
    wikiTitle:  "Library of Alexandria"
  },
  {
    id:         'T3-023',
    name:       "Bay of Pigs Invasion",
    year:       1961,
    yearMin:    1961,
    yearMax:    1961,
    lat:        22.0615,
    lng:        -81.1196,
    loc:        "Playa Gir\u00f3n, Bay of Pigs, Cuba",
    img:        'T3-023_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "20th Century AD",
    c3:         "Playa Gir\u00f3n, Bay of Pigs, Cuba, Cuba",
    desc:       "On April 17, 1961, approximately 1,400 Cuban exiles trained and equipped by the CIA landed at the Bay of Pigs on Cuba's southern coast in an attempt to overthrow Fidel Castro's government \\\\u2014 and within 72 hours had been comprehensively defeated, with over 100 dead and more than 1,100 captured, in one of the most embarrassing covert operations in American history. The plan depended on a popular uprising that never materialised and on air support that Kennedy cancelled at the last minute fearing international exposure. The failure strengthened Castro's position, pushed Cuba closer to the Soviet Union, and directly contributed to the conditions that produced the Cuban Missile Crisis eighteen months later.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/6/63/Attack_near_Playa_Giron._April_19%2C_1961._-_panoramio.jpg",
    wikiTitle:  "Bay of Pigs Invasion"
  },
  {
    id:         'T3-024',
    name:       "Signing of the Treaty of Versailles \u2013 End of WWI",
    year:       1919,
    lat:        48.8048,
    lng:        2.1203,
    loc:        "Hall of Mirrors, Palace of Versailles",
    img:        'T3-024_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Hall of Mirrors, Palace of Versailles, France",
    desc:       "On June 28, 1919 \\\\u2014 exactly five years after the assassination of Archduke Franz Ferdinand that had triggered the war \\\\u2014 representatives of Germany and the Allied powers signed the Treaty of Versailles in the Hall of Mirrors at the Palace of Versailles, formally ending the First World War and imposing on Germany sole responsibility for the conflict, massive reparations, and strict limits on its military. The German delegation signed under protest, and the treaty's punitive terms \\\\u2014 particularly the war guilt clause \\\\u2014 generated lasting resentment that Adolf Hitler would systematically exploit in his rise to power. The historian Margaret MacMillan has argued that the treaty was neither harsh enough to permanently cripple Germany nor lenient enough to reconcile it to defeat, making a second European war distinctly more likely.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b8/Treaty_of_Versailles%2C_English_version.jpg/3840px-Treaty_of_Versailles%2C_English_version.jpg",
    wikiTitle:  "Treaty of Versailles"
  },
  {
    id:         'T3-025',
    name:       "Nelson Mandela Released from Prison",
    year:       1990,
    lat:        -33.9258,
    lng:        18.4232,
    loc:        "Victor Verster Prison, Paarl, South Africa",
    img:        'T3-025_img1.png',
    yaw:        0,
    c1:         "Africa",
    c2:         "20th Century AD",
    c3:         "Victor Verster Prison, Paarl, South Africa, South Africa",
    desc:       "On February 11, 1990, Nelson Mandela walked free from Victor Verster Prison near Paarl, South Africa, after 27 years of imprisonment \\u2014 18 of them on Robben Island \\u2014 becoming at the moment of his release the most famous political prisoner in the world and the living symbol of the global struggle against apartheid. President F.W. de Klerk had announced Mandela's unconditional release nine days earlier, simultaneously unbanning the African National Congress and other anti-apartheid organisations, in the opening move of the negotiations that would end white minority rule. Mandela's release was broadcast live to an estimated one billion people worldwide; four years later he became South Africa's first democratically elected president, and his extraordinary absence of personal bitterness toward his former jailers made him a moral exemplar whose influence transcended politics.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/0/02/Nelson_Mandela_1994.jpg",
    wikiTitle:  "Nelson Mandela"
  },
  {
    id:         'T3-026',
    name:       "Constantinople Founded by Emperor Constantine",
    year:       330,
    lat:        41.0082,
    lng:        28.9784,
    loc:        "Constantinople (Istanbul), Turkey",
    img:        'T3-026_img1.png',
    yaw:        0,
    c1:         "Europe/Asia",
    c2:         "4th Century AD",
    c3:         "Constantinople (Istanbul), Turkey, Turkey",
    desc:       "On May 11, 330 AD, the Emperor Constantine I formally dedicated his new eastern capital on the strategic peninsula of Byzantium \\u2014 at the crossroads of Europe and Asia, commanding both the Bosphorus strait and the Sea of Marmara \\u2014 naming it Constantinople, City of Constantine, and modelling its seven hills and urban layout deliberately on Rome. The city Constantine built would survive the fall of the Western Roman Empire by nearly a thousand years, serving as the capital of the Byzantine Empire and the centre of Eastern Christianity until its fall to the Ottomans in 1453. At its medieval height Constantinople was the largest and wealthiest city in Europe, home to half a million people, and the repository of classical Greek and Roman knowledge that would later fuel the Renaissance.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/b/bb/Byzantine_Constantinople-en.png",
    wikiTitle:  "Constantinople"
  },
  {
    id:         'T3-027',
    name:       "Tet Offensive \u2013 Vietnam War",
    year:       1968,
    yearMin:    1968,
    yearMax:    1968,
    lat:        10.8231,
    lng:        106.6297,
    loc:        "Saigon (Ho Chi Minh City), Vietnam",
    img:        'T3-027_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "20th Century AD",
    c3:         "Saigon (Ho Chi Minh City), Vietnam, Vietnam",
    desc:       "On January 30, 1968 \\u2014 the first day of the Vietnamese lunar new year Tet \\u2014 the North Vietnamese Army and Viet Cong launched coordinated surprise attacks on more than 100 cities and military installations across South Vietnam, including a dramatic assault on the US Embassy compound in Saigon that shocked American television audiences who had been told the war was being won. Though the Tet Offensive was a catastrophic military failure for North Vietnam \\u2014 losing an estimated 45,000 fighters and gaining no lasting territory \\u2014 it was a decisive psychological and political victory, shattering American public confidence in the war's progress and accelerating the domestic opposition that would force the eventual US withdrawal. The gap between official optimism and the reality of Tet destroyed the credibility of the Johnson administration and transformed American journalism's relationship with government.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/9/96/Tet_Offensive_map.png",
    wikiTitle:  "Tet Offensive"
  },
  {
    id:         'T3-028',
    name:       "Silk Road \u2013 Tang Dynasty Trade Caravan",
    year:       618,
    yearMin:    618,
    yearMax:    907,
    lat:        39.7675,
    lng:        98.5063,
    loc:        "Dunhuang, Gansu, China (Jade Gate Pass)",
    img:        'T3-028_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "7th Century AD",
    c3:         "Dunhuang, Gansu, China (Jade Gate Pass), China",
    desc:       "For over a millennium \\u2014 from roughly the 2nd century BC to the 15th century AD \\u2014 the network of overland and maritime trade routes known as the Silk Road connected China, Central Asia, Persia, the Arabian Peninsula, East Africa, and Mediterranean Europe, carrying not only silk, spices, glass, and precious metals but also religions, technologies, diseases, and ideas across the known world. At its Tang Dynasty height around 700 AD, Chinese caravans departed from the Jade Gate Pass at Dunhuang loaded with silk, porcelain, and tea, travelling through oasis cities like Samarkand and Merv toward Persia and Byzantium, returning with gold, cotton, and glassware. The Silk Road facilitated the spread of Buddhism to China, Islam to Southeast Asia, and the bubonic plague to Europe \\u2014 making it arguably the most consequential network of human exchange in pre-modern history.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/4/49/Silk_road_Kazakhstan.svg/960px-Silk_road_Kazakhstan.svg.png",
    wikiTitle:  "Silk Road"
  },
  {
    id:         'T3-029',
    name:       "Napoleon's Retreat from Moscow",
    year:       1812,
    yearMin:    1812,
    yearMax:    1812,
    lat:        55.35,
    lng:        35.95,
    loc:        "Smolensk Road, Russia",
    img:        'T3-029_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "19th Century AD",
    c3:         "Smolensk Road, Russia, Russia",
    desc:       "In June 1812, Napoleon Bonaparte invaded Russia with the Grande Arm\\u00e9e of approximately 685,000 men \\u2014 the largest force ever assembled in European history \\u2014 and by September had captured Moscow, only to find it abandoned and burning, its supplies destroyed by the retreating Russians. Waiting for a Russian surrender that never came, Napoleon finally ordered the retreat in October; the combination of Russian harassment, scorched-earth supply denial, and one of the most savage winters in European memory reduced the Grande Arm\\u00e9e to fewer than 100,000 men by the time it crossed back into Poland. The Russian campaign broke Napoleon's aura of invincibility, destroyed the finest army in the world, and began the sequence of coalition wars that would end with his defeat at Waterloo less than three years later.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/c/c8/IV_corpo_durante_la_campagna_di_Russia.jpg",
    wikiTitle:  "French invasion of Russia"
  },
  {
    id:         'T3-030',
    name:       "Spread of Islam \u2013 Battle of Tours (Poitiers)",
    year:       732,
    lat:        46.5833,
    lng:        0.3333,
    loc:        "Between Tours and Poitiers, France",
    img:        'T3-030_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "8th Century AD",
    c3:         "Between Tours and Poitiers, France, France",
    desc:       "In October 732 AD, near the city of Tours in central France, the Frankish military commander Charles Martel \\u2014 whose name means 'the Hammer' \\u2014 halted the northernmost advance of an Umayyad Muslim army that had crossed from Spain through the Pyrenees and was raiding deep into the Frankish heartland, killing the Umayyad governor Abd al-Rahman al-Ghafiqi in a battle that lasted several days. Medieval and early modern European historians portrayed the Battle of Tours as the decisive moment that saved Christian Europe from Islamic conquest; modern historians debate whether the Umayyad force was a major invasion or a large-scale raid, and whether halting it was truly decisive for European history. What is certain is that the victory established Charles Martel as the dominant power in Western Europe and laid the foundation for the Carolingian dynasty his grandson Charlemagne would bring to its height.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/e/e7/Steuben_-_Bataille_de_Poitiers.png",
    wikiTitle:  "Battle of Tours"
  },
  {
    id:         'T3-031',
    name:       "Ancient Olympic Games",
    year:       -776,
    lat:        37.6379,
    lng:        21.63,
    loc:        "Olympia, Elis, Greece",
    img:        'T3-031_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "8th Century BC",
    c3:         "Olympia, Elis, Greece, Greece",
    desc:       "The Ancient Olympic Games were held every four years at the sanctuary of Zeus at Olympia in the western Peloponnese, traditionally from 776 BC \u2014 the first year for which a winner's name was recorded \u2014 until 393 AD when the Christian Emperor Theodosius I abolished them as pagan festivals, a run of over 1,100 years and approximately 293 Olympiads. The Games began as a single footrace of approximately 192 metres but expanded to include wrestling, discus and javelin, chariot racing, the pentathlon, and the pankration \u2014 a brutal combination of boxing and wrestling with almost no rules \u2014 and were so important to Greek culture that wars were suspended during the Olympic truce. Victors received only an olive wreath and undying fame; the social capital of an Olympic victory was enormous enough that city-states sometimes offered lifetime pensions and free meals to their champions.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/Greek_statue_discus_thrower_2_century_aC.jpg/480px-Greek_statue_discus_thrower_2_century_aC.jpg",
    wikiTitle:  "Ancient Olympic Games"
  },
  {
    id:         'T3-032',
    name:       "Battle of Yarmouk \u2013 Arab Conquest of Syria",
    year:       636,
    lat:        32.8167,
    lng:        36.1667,
    loc:        "Yarmouk River valley, Jordan/Syria border",
    img:        'T3-032_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "7th Century AD",
    c3:         "Yarmouk River valley, Jordan/Syria border, Jordan/Syria border",
    desc:       "In August 636 AD, an Arab Muslim army commanded by Khalid ibn al-Walid defeated the Byzantine Imperial Army at the Yarmouk River on the border of modern Jordan and Syria in a six-day battle that ended Byzantine control of the Levant and opened the door to the Arab conquest of Syria, Palestine, and Egypt. The Byzantine army, despite outnumbering the Arabs significantly, was hampered by ethnic and religious divisions, quarrels between commanders, and a sudden dust storm on the final day that blinded their troops. The Battle of Yarmouk is considered one of the most decisive battles in world history \u2014 it permanently ended the 700-year Roman and Byzantine presence in the eastern Mediterranean and accelerated the spread of Islam across the entire region within a generation.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/d/d1/Hayton_BNF886_9v.jpg",
    wikiTitle:  "Battle of the Yarmuk"
  },
  {
    id:         'T3-033',
    name:       "Maya City of Tikal at Its Height",
    year:       650,
    yearMin:    650,
    yearMax:    800,
    lat:        17.222,
    lng:        -89.6237,
    loc:        "Tikal, Pet\u00e9n, Guatemala",
    img:        'T3-033_img1.png',
    yaw:        0,
    c1:         "North America",
    c2:         "7th Century AD",
    c3:         "Tikal, Pet\u00e9n, Guatemala, Guatemala",
    desc:       "At its height around 700 AD, the Maya city of Tikal in what is now northern Guatemala was one of the largest cities in the ancient world, home to an estimated 100,000 to 200,000 people and dominated by six massive temple-pyramids rising above the rainforest canopy \u2014 the tallest, Temple IV, standing 65 metres high and still visible for miles. Tikal had been a major Maya centre since the first century BC and its power rested on control of trade routes, military conquest of rival city-states, and a sophisticated agricultural system of raised fields and reservoirs that sustained its enormous population. The city was abandoned around 900 AD as part of the broader Maya Classic collapse \u2014 a catastrophic civilisational decline triggered by drought, warfare, deforestation, and political fragmentation whose causes are still debated.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/7/75/Tikal_3.jpg",
    wikiTitle:  "Tikal"
  },
  {
    id:         'T3-034',
    name:       "Benin Kingdom \u2013 Royal Court of the Oba",
    year:       1450,
    yearMin:    1450,
    yearMax:    1550,
    lat:        6.335,
    lng:        5.627,
    loc:        "Benin City, Edo State, Nigeria",
    img:        'T3-034_img1.png',
    yaw:        0,
    c1:         "Africa",
    c2:         "15th Century AD",
    c3:         "Benin City, Edo State, Nigeria, Nigeria",
    desc:       "The Benin Kingdom, centred on Benin City in what is now southern Nigeria and reaching the height of its power around 1500 AD, was one of the most sophisticated and artistically accomplished states in pre-colonial Africa \u2014 a highly centralised monarchy ruled by the divine Oba, supported by an elaborate bureaucracy and defended by a professional army, whose royal court produced the extraordinary Benin Bronzes: thousands of plaques and sculptures cast in brass using techniques of lost-wax casting that astonished European metallurgists when they first encountered them in 1897. The kingdom controlled trade across the Niger Delta and the Bight of Benin, exchanging pepper, cloth, and ivory for European copper and firearms. When British forces sacked Benin City in 1897 and looted over 3,000 bronze and ivory artworks, they dispersed them to museums across Europe and America \u2014 where debates over their repatriation continue today.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/3/3b/Benin_Bronzes_-_Museum_f%C3%BCr_V%C3%B6lkerkunde_Berlin.jpg/640px-Benin_Bronzes_-_Museum_f%C3%BCr_V%C3%B6lkerkunde_Berlin.jpg",
    wikiTitle:  "Kingdom of Benin"
  },
  {
    id:         'T3-035',
    name:       "Polynesian Navigation \u2013 Settlement of Hawaii",
    year:       300,
    yearMin:    300,
    yearMax:    600,
    lat:        19.8968,
    lng:        -155.5828,
    loc:        "Hawaii (Big Island), Pacific Ocean",
    img:        'T3-035_img1.png',
    yaw:        0,
    c1:         "Oceania/Pacific",
    c2:         "3rd Century AD",
    c3:         "Hawaii (Big Island), Pacific Ocean, USA (Hawaii)",
    desc:       "Around 400 AD, Polynesian voyagers arrived in the Hawaiian Islands \u2014 the most remote island chain on Earth, more than 3,000 kilometres from the nearest inhabited land \u2014 completing the most remarkable feat of open-ocean navigation in human history, crossing thousands of miles of open Pacific in double-hulled canoes guided by stars, wave patterns, bird behaviour, cloud formations, and the colour of the sea, without instruments of any kind. The Polynesians, who had already settled Fiji, Tonga, Samoa, the Society Islands, and Easter Island using the same techniques, brought with them the plants, animals, and cultural practices that would form the foundation of Hawaiian civilisation. The precision of their navigation has been confirmed by modern experimental voyages using traditional methods, which have demonstrated that it was a disciplined science rather than accidental drift.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/2/21/Hokule%27aSailing2009.jpg",
    wikiTitle:  "Polynesian navigation"
  },
  {
    id:         'T3-036',
    name:       "Suleiman the Magnificent \u2013 Siege of Vienna",
    year:       1529,
    lat:        48.2082,
    lng:        16.3738,
    loc:        "Vienna, Austria",
    img:        'T3-036_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "16th Century AD",
    c3:         "Vienna, Austria, Austria",
    desc:       "In the summer of 1529, the Ottoman Sultan Suleiman the Magnificent led an army of approximately 120,000 soldiers on a campaign that reached the walls of Vienna \u2014 the westernmost point of Ottoman military penetration into Europe \u2014 and laid siege to the Habsburg capital for three weeks before withdrawing in mid-October as the onset of winter, supply exhaustion, and stubborn resistance made continuing untenable. The siege marked the high-water mark of Ottoman expansion into central Europe: Vienna was never successfully besieged again, and the frontier between the Ottoman and Habsburg empires would stabilise roughly along the line it held in 1529 for the next 150 years. The failed siege entered European consciousness as the moment Christendom narrowly averted Ottoman conquest, shaping the cultural memory of conflict between Islam and Europe for centuries.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/f/f6/Siegeofvienna1529.jpg",
    wikiTitle:  "Siege of Vienna (1529)"
  },
  {
    id:         'T3-037',
    name:       "Cleopatra Meets Julius Caesar in Alexandria",
    year:       -48,
    lat:        31.1975,
    lng:        29.8925,
    loc:        "Royal Palace Quarter, Alexandria, Egypt",
    img:        'T3-037_img1.png',
    yaw:        0,
    c1:         "Africa",
    c2:         "1st Century BC",
    c3:         "Royal Palace Quarter, Alexandria, Egypt, Egypt",
    desc:       "In the winter of 48 BC, the 21-year-old Queen Cleopatra VII \u2014 who had recently been driven from her throne by her younger brother Ptolemy XIII and was technically an enemy of Rome \u2014 had herself smuggled past guards into the royal palace in Alexandria inside a rolled carpet or linen sack, according to Plutarch, to meet Julius Caesar, the most powerful man in the Roman world, who had arrived in Egypt pursuing his defeated rival Pompey. The meeting, which both parties found immediately advantageous \u2014 Caesar needed Egyptian grain and money, Cleopatra needed Roman military support to retake her throne \u2014 rapidly became a personal and political alliance that produced a son, Caesarion, and reshaped the eastern Mediterranean. Cleopatra's relationship with Caesar, and later with Mark Antony, transformed her into the most mythologised woman of antiquity.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/3/3e/Kleopatra-VII.-Altes-Museum-Berlin1.jpg",
    wikiTitle:  "Cleopatra"
  },
  {
    id:         'T3-038',
    name:       "Mongol Invasion of Japan \u2013 Divine Wind (Kamikaze)",
    year:       1281,
    lat:        33.5904,
    lng:        130.399,
    loc:        "Hakata Bay, Kyushu, Japan",
    img:        'T3-038_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "13th Century AD",
    c3:         "Hakata Bay, Kyushu, Japan, Japan",
    desc:       "In August 1281, the largest naval invasion fleet ever assembled to that date \u2014 an estimated 4,400 ships and 140,000 Mongol, Korean, and Chinese soldiers sent by Kublai Khan \u2014 arrived off Kyushu intent on conquering Japan, which had repelled a first invasion attempt seven years earlier. After weeks of fighting the beachhead at Hakata Bay without achieving a decisive breakthrough against determined Japanese samurai resistance, the invasion fleet was struck by a massive typhoon that sank an estimated 4,000 ships and killed between 30,000 and 60,000 soldiers in a single night. The Japanese attributed their salvation to a divine wind \u2014 kamikaze \u2014 sent by the gods to protect their islands, a myth of divine protection that would profoundly shape Japanese national identity and resurface, with fatal consequences, in the Second World War.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/a/a1/Mongol_invasions_of_Japan_1274%2C_1281.jpg",
    wikiTitle:  "Mongol invasions of Japan"
  },
  {
    id:         'T3-039',
    name:       "Martin Luther Posts the 95 Theses",
    year:       1517,
    lat:        51.8663,
    lng:        12.642,
    loc:        "Castle Church, Wittenberg, Germany",
    img:        'T3-039_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "16th Century AD",
    c3:         "Castle Church, Wittenberg, Germany, Germany",
    desc:       "On October 31, 1517, the Augustinian friar Martin Luther reportedly nailed his Ninety-Five Theses \u2014 a list of arguments against the Catholic Church's sale of indulgences, which promised buyers remission from punishment for sins \u2014 to the door of the Castle Church in Wittenberg, Germany, a customary academic notice board for scholarly debate. Luther intended an internal theological dispute within the Church; instead, the theses were translated from Latin into German, printed on the newly invented printing press, and distributed across Europe within weeks, igniting a religious revolution that would split Western Christianity permanently and trigger nearly a century of religious warfare. The Protestant Reformation Luther's protest launched transformed not only theology but literacy, education, nationalism, and the political organisation of Europe.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/3/3a/Luther_95_Thesen.png",
    wikiTitle:  "Ninety-five Theses"
  },
  {
    id:         'T3-040',
    name:       "First Pan-African Congress",
    year:       1919,
    yearMin:    1919,
    yearMax:    1919,
    lat:        48.8566,
    lng:        2.3522,
    loc:        "Grand H\u00f4tel, Paris, France",
    img:        'T3-040_img1.png',
    yaw:        0,
    c1:         "Europe",
    c2:         "20th Century AD",
    c3:         "Grand H\u00f4tel, Paris, France, France",
    desc:       "In February 1919, W.E.B. Du Bois organised the first Pan-African Congress at the Grand H\u00f4tel in Paris \u2014 convened deliberately to coincide with the Paris Peace Conference at which the victorious Allied powers were redrawing the world map \u2014 bringing together 57 delegates from 15 countries to demand self-governance for African colonies and equal rights for people of African descent worldwide. The Congress was held over French government objections and with the active opposition of the American government, which feared its influence on colonial subjects; Du Bois had to secure special permission through the intervention of the Senegalese deputy Blaise Diagne. The 1919 Congress launched the Pan-African movement that would, over the following four decades, provide the intellectual and organisational foundation for the decolonisation of Africa and the Caribbean.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/thumb/2/29/WEB_DuBois_1918.jpg/400px-WEB_DuBois_1918.jpg",
    wikiTitle:  "Pan-African Congress"
  },
  {
    id:         'T3-041',
    name:       "Zheng He's Treasure Fleet Voyage",
    year:       1405,
    yearMin:    1405,
    yearMax:    1407,
    lat:        1.3521,
    lng:        103.8198,
    loc:        "Malacca Strait / Singapore region",
    img:        'T3-041_img1.png',
    yaw:        0,
    c1:         "Asia",
    c2:         "15th Century AD",
    c3:         "Malacca Strait / Singapore region, Malaysia/Singapore (sea route)",
    desc:       "Between 1405 and 1433, the Chinese Admiral Zheng He commanded seven extraordinary voyages from the Ming imperial capital of Nanjing across the South China Sea and Indian Ocean, reaching as far as the Persian Gulf, the Red Sea, and the East African coast as far south as Malindi in modern Kenya \u2014 commanding fleets of up to 317 ships, including the 'treasure ships' estimated at 120 to 137 metres in length, the largest wooden ships ever built, and carrying up to 28,000 sailors and soldiers. The voyages were not primarily trading expeditions but diplomatic missions projecting Chinese imperial power: Zheng He collected tribute, installed friendly rulers, and brought back exotic animals including giraffes, which the Chinese court interpreted as the mythical qilin, a sign of divine favour. The voyages ended abruptly after 1433 when Confucian court factions triumphed over the eunuch admirals, China turned inward, and the treasure fleets were deliberately destroyed \u2014 in what historians consider one of history's most consequential acts of deliberate self-isolation.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/0/0e/2016_Malakka%2C_Stadhuys_%2809%29.jpg",
    wikiTitle:  "Zheng He"
  },
  {
    id:         'T3-042',
    name:       "Transatlantic Slave Trade \u2013 Middle Passage",
    year:       1650,
    yearMin:    1650,
    yearMax:    1800,
    lat:        6.3698,
    lng:        -2.4194,
    loc:        "West African coast (Ouidah, Benin)",
    img:        'T3-042_img1.png',
    yaw:        0,
    c1:         "Africa",
    c2:         "17th Century AD",
    c3:         "West African coast (Ouidah, Benin), Benin (formerly Dahomey)",
    desc:       "Between approximately 1500 and 1900, the transatlantic slave trade forcibly transported an estimated 12.5 million African men, women, and children from West and Central Africa to the Americas, of whom approximately 1.8 million died during the Middle Passage \u2014 the horrific ocean crossing in which enslaved people were packed into ships with insufficient food, water, or space to stand. At the trade's peak around 1700, the port of Ouidah in the Kingdom of Dahomey (modern Benin) was among the largest slave-trading ports in Africa, dispatching tens of thousands of people annually through what survivors called the Door of No Return. The slave trade and the plantation economies it sustained generated enormous wealth for European powers and their American colonies while simultaneously destroying the demographic, economic, and social fabric of entire regions of Africa \u2014 consequences whose effects persist in the structural inequalities of the present day.",
    wikiImg:    "https://upload.wikimedia.org/wikipedia/commons/1/16/Slave_Auction_Ad.jpg",
    wikiTitle:  "Atlantic slave trade"
  }
];
